...
spa_ld_claim_log_blocks(spa_t *spa)
{
	dmu_tx_t *tx;
	dsl_pool_t *dp = spa_get_dsl(spa);
	zfs_dbgmsg(" Claim log blocks that haven't been committed yet ..\n");
	/*
...
spa_load_impl(spa_t *spa, spa_import_type_t type, const char **ereport)
{
	...
	zfs_dbgmsg( " it also traverses and claims the ZIL blocks\n");
	int error = 0;
...
spa_sync_rewrite_vdev_config(spa_t *spa, dmu_tx_t *tx)
{
	...
	vdev_t *rvd = spa->spa_root_vdev;
	uint64_t txg = tx->tx_txg;
	zfs_dbgmsg(" rewrite the vdev configuration (which includes the uberblock)\
		to commit the transaction group txg=%llu\n", (u_longlong_t)txg);
...
spa_sync(spa_t *spa, uint64_t txg)
{
	...
	// avoid cleaning up so we can have all ZIL commiments since not all of them are changed
...
//	cleanup_cmts(&zils_blocks_commitments);
	zfs_dbgmsg(" ***** sync txg=%llu end *****\n", (u_longlong_t)txg);
}
