    ...
    boolean_t checkpoint)
{
	...
	if (checkpoint) {
		...
		// zfs_dbgmsg(" range_tree_add(ms_freeing)\n");
    ...
    boolean_t checkpoint)
{
	...
	if (spa->spa_vdev_removal != NULL &&
		 ...
		 */
		zfs_dbgmsg(" free_from_removing_vdev\n");
		free_from_removing_vdev(vd, offset, size);
	} else if (vd->vdev_ops->vdev_op_remap != NULL) {
		zfs_dbgmsg(" vdev_indirect_mark_obsolete\n");
		vdev_indirect_mark_obsolete(vd, offset, size);
		vd->vdev_ops->vdev_op_remap(vd, offset, size,
		    metaslab_free_impl_cb, &checkpoint);
	} else {
		// zfs_dbgmsg(" metaslab_free_concrete\n");
...
metaslab_free_dva(spa_t *spa, const dva_t *dva, boolean_t checkpoint)
{
	...
	// zfs_dbgmsg(" DVA=<%llu:%llx:%llx>", (u_longlong_t)DVA_GET_VDEV(dva),  (u_longlong_t)DVA_GET_OFFSET(dva),		
...
metaslab_claim_dva(spa_t *spa, const dva_t *dva, uint64_t txg)
{
	...
	// zfs_dbgmsg(" txg=%llu, DVA=<%llu:%llx:%llx>\n", (u_longlong_t)txg, (u_longlong_t)DVA_GET_VDEV(dva),  (u_longlong_t)DVA_GET_OFFSET(dva),		
	//		    (u_longlong_t)DVA_GET_ASIZE(dva));
