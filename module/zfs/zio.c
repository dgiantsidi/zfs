...
static int zio_kstats_update(kstat_t *ksp, int rw) {
...
void zio_init(void) {
  ...
  zio_cache = kmem_cache_create("zio_cache", sizeof(zio_t), 0, NULL, NULL, NULL,
                                NULL, NULL, 0);
  zio_link_cache = kmem_cache_create("zio_link_cache", sizeof(zio_link_t), 0,
  ...
  zio_ksp = kstat_create("zfs", 0, "zio_stats", "misc", KSTAT_TYPE_NAMED,
                         sizeof(zio_stats) / sizeof(kstat_named_t),
                         KSTAT_FLAG_VIRTUAL);
  if (zio_ksp != NULL) {
  ...
  for (c = 0; c < SPA_MAXBLOCKSIZE >> SPA_MINBLOCKSHIFT; c++) {
    ...
    cflags =
        (zio_exclude_metadata || size > zio_buf_debug_limit) ? KMC_NODEBUG : 0;
    ...
    if (cflags == data_cflags) {
       ...
       */
      (void)snprintf(name, sizeof(name), "zio_buf_comb_%lu", (ulong_t)size);
      zio_buf_cache[c] = kmem_cache_create(name, size, align, NULL, NULL, NULL,
                                           NULL, NULL, cflags);
    ...
    zio_data_buf_cache[c] = kmem_cache_create(name, size, align, NULL, NULL,
                                              NULL, NULL, NULL, data_cflags);
  }
 ...
 */
static void zio_buf_put_canary(ulong_t *p, size_t size, kmem_cache_t **cache,
                               size_t c) {
  size_t off = P2ROUNDUP(size, sizeof(ulong_t));
  ulong_t *canary = p + off / sizeof(ulong_t);
  size_t asize = (c + 1) << SPA_MINBLOCKSHIFT;
  if (c + 1 < SPA_MAXBLOCKSIZE >> SPA_MINBLOCKSHIFT && cache[c] == cache[c + 1])
    asize = (c + 2) << SPA_MINBLOCKSHIFT;
  for (; off < asize; canary++, off += sizeof(ulong_t))
    *canary = zio_buf_canary;
}

static void zio_buf_check_canary(ulong_t *p, size_t size, kmem_cache_t **cache,
                                 size_t c) {
  size_t off = P2ROUNDUP(size, sizeof(ulong_t));
  ulong_t *canary = p + off / sizeof(ulong_t);
  size_t asize = (c + 1) << SPA_MINBLOCKSHIFT;
  ...
  for (; off < asize; canary++, off += sizeof(ulong_t)) {
    if (unlikely(*canary != zio_buf_canary)) {
      PANIC("ZIO buffer overflow %p (%zu) + %zu %#lx != %#lx", p, size,
            (canary - p) * sizeof(ulong_t), *canary, zio_buf_canary);
    }
  }
}
#endif
...
static void zio_abd_free(void *abd, size_t size) {
  (void)size;
  abd_free((abd_t *)abd);
}
 ...
 * ==========================================================================
 */
void zio_push_transform(zio_t *zio, abd_t *data, uint64_t size,
...
void zio_pop_transforms(zio_t *zio) {
  ...
  while ((zt = zio->io_transform_stack) != NULL) {
    if (zt->zt_transform != NULL)
      zt->zt_transform(zio, zt->zt_orig_abd, zt->zt_orig_size);
 ...
 * ==========================================================================
 */
static void zio_subblock(zio_t *zio, abd_t *data, uint64_t size) {
...
static void zio_decompress(zio_t *zio, abd_t *data, uint64_t size) {
  if (zio->io_error == 0) {
    int ret =
        zio_decompress_data(BP_GET_COMPRESS(zio->io_bp), zio->io_abd, data,
                            zio->io_size, size, &zio->io_prop.zp_complevel);
...
static void zio_decrypt(zio_t *zio, abd_t *data, uint64_t size) {
  ...
  if (BP_HAS_INDIRECT_MAC_CKSUM(bp)) {
    ...
    if (BP_GET_COMPRESS(bp) != ZIO_COMPRESS_OFF) {
      ...
      ret =
          zio_decompress_data(BP_GET_COMPRESS(bp), zio->io_abd, abd,
                              zio->io_size, lsize, &zio->io_prop.zp_complevel);
      if (ret != 0) {
        abd_free(abd);
        ret = SET_ERROR(EIO);
        goto error;
      }
      ret = zio_crypt_do_indirect_mac_checksum_abd(B_FALSE, abd, lsize,
                                                   BP_SHOULD_BYTESWAP(bp), mac);
      abd_free(abd);
    } else {
      ret = zio_crypt_do_indirect_mac_checksum_abd(B_FALSE, zio->io_abd, size,
                                                   BP_SHOULD_BYTESWAP(bp), mac);
    }
    abd_copy(data, zio->io_abd, size);

    if (zio_injection_enabled && ot != DMU_OT_DNODE && ret == 0) {
      ret = zio_handle_decrypt_injection(spa, &zio->io_bookmark, ot, ECKSUM);
    }
    if (ret != 0)
      goto error;
  ...
  if (BP_IS_AUTHENTICATED(bp)) {
    if (ot == DMU_OT_OBJSET) {
      ret = spa_do_crypt_objset_mac_abd(B_FALSE, spa, dsobj, zio->io_abd, size,
                                        BP_SHOULD_BYTESWAP(bp));
    } else {
      zio_crypt_decode_mac_bp(bp, mac);
      ret = spa_do_crypt_mac_abd(B_FALSE, spa, dsobj, zio->io_abd, size, mac);
      if (zio_injection_enabled && ret == 0) {
        ret = zio_handle_decrypt_injection(spa, &zio->io_bookmark, ot, ECKSUM);
      }
    }
  ...
  ret = spa_do_crypt_abd(B_FALSE, spa, &zio->io_bookmark, BP_GET_TYPE(bp),
                         BP_GET_DEDUP(bp), BP_SHOULD_BYTESWAP(bp), salt, iv,
                         mac, size, data, zio->io_abd, &no_crypt);
  if (no_crypt)
    abd_copy(data, zio->io_abd, size);
...
error:
  ...
  if (ret == ECKSUM) {
    ...
    if ((zio->io_flags & ZIO_FLAG_SPECULATIVE) == 0) {
      spa_log_error(spa, &zio->io_bookmark, BP_GET_LOGICAL_BIRTH(zio->io_bp));
      (void)zfs_ereport_post(FM_EREPORT_ZFS_AUTHENTICATION, spa, NULL,
 ...
 * ==========================================================================
 */
zio_t *zio_walk_parents(zio_t *cio, zio_link_t **zl) {
...
zio_t *zio_walk_children(zio_t *pio, zio_link_t **zl) {
...
void zio_add_child_first(zio_t *pio, zio_t *cio) {
  /*
   * Logical I/Os can have logical, gang, or vdev children.
   * Gang I/Os can have gang or vdev children.
   * Vdev I/Os can only have vdev children.
...
static boolean_t zio_wait_for_children(zio_t *zio, uint8_t childbits,
...
zio_notify_parent(zio_t *pio, zio_t *zio, enum zio_wait_type wait,
  ...
  if (*countp == 0 && pio->io_stall == countp) {
    zio_taskq_type_t type = pio->io_stage < ZIO_STAGE_VDEV_IO_START
                                ? ZIO_TASKQ_ISSUE
                                : ZIO_TASKQ_INTERRUPT;
    pio->io_stall = NULL;
    mutex_exit(&pio->io_lock);
 ...
 * ==========================================================================
 */
static zio_t *zio_create(zio_t *pio, spa_t *spa, uint64_t txg,
                         const blkptr_t *bp, abd_t *data, uint64_t lsize,
                         uint64_t psize, zio_done_func_t *done, void *private,
                         zio_type_t type, zio_priority_t priority,
  ...
  if (bp != NULL) {
    if (type != ZIO_TYPE_WRITE || zio->io_child_type == ZIO_CHILD_DDT) {
      zio->io_bp_copy = *bp;
  ...
  zio->io_state[ZIO_WAIT_READY] =
      (stage >= ZIO_STAGE_READY) || (pipeline & ZIO_STAGE_READY) == 0;
 ...
 */
zio_t *zio_null(zio_t *pio, spa_t *spa, vdev_t *vd, zio_done_func_t *done,
                void *private, zio_flag_t flags) {
  zio_t *zio;

  zio = zio_create(pio, spa, 0, NULL, NULL, 0, 0, done, private, ZIO_TYPE_NULL,
                   ZIO_PRIORITY_NOW, flags, vd, 0, NULL, ZIO_STAGE_OPEN,
 ...
 * (any other ZIO type).
 */
zio_t *zio_root(spa_t *spa, zio_done_func_t *done, void *private,
                zio_flag_t flags) {
  zio_t *zio;

  zio = zio_create(NULL, spa, 0, NULL, NULL, 0, 0, done, private, ZIO_TYPE_NULL,
                   ZIO_PRIORITY_NOW, flags, NULL, 0, NULL, ZIO_STAGE_OPEN,
                   ZIO_ROOT_PIPELINE);

  return (zio);
}

static int zfs_blkptr_verify_log(spa_t *spa, const blkptr_t *bp,
  ...
  zfs_dbgmsg(
      ...
      bp, (long long)bp->blk_dva[0].dva_word[0],
      ...
      (long long)bp->blk_dva[2].dva_word[1], (long long)bp->blk_prop,
      (long long)bp->blk_pad[0], (long long)bp->blk_pad[1],
      (long long)BP_GET_PHYSICAL_BIRTH(bp), (long long)BP_GET_LOGICAL_BIRTH(bp),
      (long long)bp->blk_fill, (long long)bp->blk_cksum.zc_word[0],
      (long long)bp->blk_cksum.zc_word[1], (long long)bp->blk_cksum.zc_word[2],
 ...
 *   performance
 */
int zfs_blkptr_verify(spa_t *spa, const blkptr_t *bp,
                      enum blk_config_flag blk_config,
                      enum blk_verify_flag blk_verify) {
  int errors = 0;

  if (unlikely(!DMU_OT_IS_VALID(BP_GET_TYPE(bp)))) {
    errors += zfs_blkptr_verify_log(spa, bp, blk_verify,
                                    "blkptr at %px has invalid TYPE %llu", bp,
                                    (longlong_t)BP_GET_TYPE(bp));
  }
  ...
  if (unlikely(BP_GET_LSIZE(bp) > SPA_MAXBLOCKSIZE)) {
    errors += zfs_blkptr_verify_log(spa, bp, blk_verify,
                                    "blkptr at %px has invalid LSIZE %llu", bp,
                                    (longlong_t)BP_GET_LSIZE(bp));
  }
  ...
  if (unlikely(BP_GET_PSIZE(bp) > SPA_MAXBLOCKSIZE)) {
    errors += zfs_blkptr_verify_log(spa, bp, blk_verify,
                                    "blkptr at %px has invalid PSIZE %llu", bp,
  ...
  for (int i = 0; i < BP_GET_NDVAS(bp); i++) {
    ...
    if (unlikely(vdevid >= spa->spa_root_vdev->vdev_children)) {
      errors += zfs_blkptr_verify_log(
          spa, bp, blk_verify, "blkptr at %px DVA %u has invalid VDEV %llu", bp,
          i, (longlong_t)vdevid);
      continue;
    }
    ...
    if (unlikely(offset + asize > vd->vdev_asize)) {
      errors += zfs_blkptr_verify_log(
          spa, bp, blk_verify, "blkptr at %px DVA %u has invalid OFFSET %llu",
          bp, i, (longlong_t)offset);
    }
  }
  if (blk_config == BLK_CONFIG_NEEDED || blk_config == BLK_CONFIG_NEEDED_TRY)
    spa_config_exit(spa, SCL_VDEV, bp);

  return (errors ? ECKSUM : 0);
}

boolean_t zfs_dva_valid(spa_t *spa, const dva_t *dva, const blkptr_t *bp) {
  (void)bp;
  uint64_t vdevid = DVA_GET_VDEV(dva);
...
zio_t *zio_read(zio_t *pio, spa_t *spa, const blkptr_t *bp, abd_t *data,
                uint64_t size, zio_done_func_t *done, void *private,
                zio_priority_t priority, zio_flag_t flags,
                const zbookmark_phys_t *zb) {
  zio_t *zio;

  zio = zio_create(pio, spa, BP_GET_BIRTH(bp), bp, data, size, size, done,
                   private, ZIO_TYPE_READ, priority, flags, NULL, 0, zb,
                   ZIO_STAGE_OPEN,
...
zio_t *zio_write(zio_t *pio, spa_t *spa, uint64_t txg, blkptr_t *bp,
                 abd_t *data, uint64_t lsize, uint64_t psize,
                 const zio_prop_t *zp, zio_done_func_t *ready,
                 zio_done_func_t *children_ready, zio_done_func_t *done,
                 void *private, zio_priority_t priority, zio_flag_t flags,
                 const zbookmark_phys_t *zb) {
  zio_t *zio;
  enum zio_stage pipeline =
      zp->zp_direct_write == B_TRUE  ? ZIO_DIRECT_WRITE_PIPELINE
      : (flags & ZIO_FLAG_DDT_CHILD) ? ZIO_DDT_CHILD_WRITE_PIPELINE
                                     : ZIO_WRITE_PIPELINE;

  zio = zio_create(pio, spa, txg, bp, data, lsize, psize, done, private,
                   ZIO_TYPE_WRITE, priority, flags, NULL, 0, zb, ZIO_STAGE_OPEN,
                   pipeline);
...
zio_t *zio_rewrite(zio_t *pio, spa_t *spa, uint64_t txg, blkptr_t *bp,
                   abd_t *data, uint64_t size, zio_done_func_t *done,
                   void *private, zio_priority_t priority, zio_flag_t flags,
                   zbookmark_phys_t *zb) {
  zio_t *zio;

  zio = zio_create(pio, spa, txg, bp, data, size, size, done, private,
                   ZIO_TYPE_WRITE, priority, flags | ZIO_FLAG_IO_REWRITE, NULL,
                   0, zb, ZIO_STAGE_OPEN, ZIO_REWRITE_PIPELINE);
...
void zio_free(spa_t *spa, uint64_t txg, const blkptr_t *bp) {
   ...
   */
  if (BP_IS_GANG(bp) || BP_GET_DEDUP(bp) || txg != spa->spa_syncing_txg ||
 ...
 */
zio_t *zio_free_sync(zio_t *pio, spa_t *spa, uint64_t txg, const blkptr_t *bp,
                     zio_flag_t flags) {
  ...
  if (BP_IS_GANG(bp) || BP_GET_DEDUP(bp) || brt_maybe_exists(spa, bp)) {
    /*
     ...
     */
    enum zio_stage stage = ZIO_FREE_PIPELINE | ZIO_STAGE_ISSUE_ASYNC;
...
zio_t *zio_claim(zio_t *pio, spa_t *spa, uint64_t txg, const blkptr_t *bp,
                 zio_done_func_t *done, void *private, zio_flag_t flags) {
  zio_t *zio;

  (void)zfs_blkptr_verify(spa, bp,
                          (flags & ZIO_FLAG_CONFIG_WRITER) ? BLK_CONFIG_HELD
                                                           : BLK_CONFIG_NEEDED,
                          BLK_VERIFY_HALT);
  ...
  zio = zio_create(pio, spa, txg, bp, NULL, BP_GET_PSIZE(bp), BP_GET_PSIZE(bp),
                   done, private, ZIO_TYPE_CLAIM, ZIO_PRIORITY_NOW, flags, NULL,
                   0, NULL, ZIO_STAGE_OPEN, ZIO_CLAIM_PIPELINE);
  ASSERT0(zio->io_queued_timestamp);

  return (zio);
}

zio_t *zio_trim(zio_t *pio, vdev_t *vd, uint64_t offset, uint64_t size,
  ...
  zio = zio_create(pio, vd->vdev_spa, 0, NULL, NULL, size, size, done, private,
                   ZIO_TYPE_TRIM, priority, flags | ZIO_FLAG_PHYSICAL, vd,
                   offset, NULL, ZIO_STAGE_OPEN, ZIO_TRIM_PIPELINE);
  zio->io_trim_flags = trim_flags;

  return (zio);
}

zio_t *zio_read_phys(zio_t *pio, vdev_t *vd, uint64_t offset, uint64_t size,
                     abd_t *data, int checksum, zio_done_func_t *done,
                     void *private, zio_priority_t priority, zio_flag_t flags,
                     boolean_t labels) {
  zio_t *zio;
  ...
  zio = zio_create(pio, vd->vdev_spa, 0, NULL, data, size, size, done, private,
                   ZIO_TYPE_READ, priority, flags | ZIO_FLAG_PHYSICAL, vd,
...
zio_t *zio_write_phys(zio_t *pio, vdev_t *vd, uint64_t offset, uint64_t size,
                      abd_t *data, int checksum, zio_done_func_t *done,
                      void *private, zio_priority_t priority, zio_flag_t flags,
                      boolean_t labels) {
  zio_t *zio;
  ...
  zio = zio_create(pio, vd->vdev_spa, 0, NULL, data, size, size, done, private,
                   ZIO_TYPE_WRITE, priority, flags | ZIO_FLAG_PHYSICAL, vd,
 ...
 */
zio_t *zio_vdev_child_io(zio_t *pio, blkptr_t *bp, vdev_t *vd, uint64_t offset,
                         abd_t *data, uint64_t size, int type,
                         zio_priority_t priority, zio_flag_t flags,
  ...
  } else if (type == ZIO_TYPE_WRITE && pio->io_prop.zp_direct_write == B_TRUE) {
    /*
  ...
  zio = zio_create(pio, pio->io_spa, pio->io_txg, bp, data, size, size, done,
                   private, type, priority, flags, vd, offset,
...
zio_t *zio_vdev_delegated_io(vdev_t *vd, uint64_t offset, abd_t *data,
  ...
  zio = zio_create(
      NULL, vd->vdev_spa, 0, NULL, data, size, size, done, private, type,
      priority,
      flags | ZIO_FLAG_CANFAIL | ZIO_FLAG_DONT_RETRY | ZIO_FLAG_DELEGATED, vd,
      offset, NULL, ZIO_STAGE_VDEV_IO_START >> 1, ZIO_VDEV_CHILD_PIPELINE);
 ...
 */
void zio_flush(zio_t *pio, vdev_t *vd) {
  ...
  if (vd->vdev_children == 0) {
    zio_nowait(zio_create(pio, vd->vdev_spa, 0, NULL, NULL, 0, 0, NULL, NULL,
                          ZIO_TYPE_FLUSH, ZIO_PRIORITY_NOW, flags, vd, 0, NULL,
 ...
 */
static uint64_t zio_roundup_alloc_size(spa_t *spa, uint64_t size) {
  if (size > spa->spa_min_alloc)
    return (roundup(size, spa->spa_gcd_alloc));
  return (spa->spa_min_alloc);
}

size_t zio_get_compression_max_size(enum zio_compress compress,
                                    uint64_t gcd_alloc, uint64_t min_alloc,
                                    size_t s_len) {
  size_t d_len;
...
static zio_t *zio_read_bp_init(zio_t *zio) {
  ...
  if (BP_GET_COMPRESS(bp) != ZIO_COMPRESS_OFF &&
    ...
    zio_push_transform(zio, abd_alloc_sametype(zio->io_abd, psize), psize,
...
static zio_t *zio_write_bp_init(zio_t *zio) {
  ...
  if (zio->io_bp_override) {
    ...
    *bp = *zio->io_bp_override;
    ...
    ASSERT(
        (zio_checksum_table[zp->zp_checksum].ci_flags & ZCHECKSUM_FLAG_DEDUP) ||
        zp->zp_dedup_verify);

    if (BP_GET_CHECKSUM(bp) == zp->zp_checksum && !zp->zp_encrypt) {
      BP_SET_DEDUP(bp, 1);
...
static zio_t *zio_write_compress(zio_t *zio) {
  spa_t *spa = zio->io_spa;
  zio_prop_t *zp = &zio->io_prop;
   ...
   */
  if (zio_wait_for_children(zio, ZIO_CHILD_LOGICAL_BIT | ZIO_CHILD_GANG_BIT,
                            ZIO_WAIT_READY)) {
    return (NULL);
  }
  ...
  if (!BP_IS_HOLE(bp) && BP_GET_LOGICAL_BIRTH(bp) == zio->io_txg) {
    ...
    ASSERT(BP_IS_EMBEDDED(bp) || BP_IS_GANG(bp) ||
           MIN(zp->zp_copies, spa_max_replication(spa)) == BP_GET_NDVAS(bp));
  }
  ...
  if (compress != ZIO_COMPRESS_OFF &&
    ...
    if (abd_cmp_zero(zio->io_abd, lsize) == 0)
      ...
      psize = zio_compress_data(
          compress, zio->io_abd, &cabd, lsize,
          zio_get_compression_max_size(compress, spa->spa_gcd_alloc,
    ...
    if (psize == 0) {
      ...
      encode_embedded_bp_compressed(bp, cbuf, compress, lsize, psize);
      ...
      zio->io_pipeline = ZIO_INTERLOCK_PIPELINE;
      ASSERT(spa_feature_is_active(spa, SPA_FEATURE_EMBEDDED_DATA));
      return (zio);
    } else {
      /*
       ...
       * the last sector.
       */
      size_t rounded = (size_t)zio_roundup_alloc_size(spa, psize);
      if (rounded >= lsize) {
        compress = ZIO_COMPRESS_OFF;
        abd_free(cabd);
        psize = lsize;
        ...
        psize = rounded;
        zio_push_transform(zio, cabd, psize, lsize, NULL);
      }
    }
    ...
    *bp = zio->io_bp_orig;
     ...
     */
    size_t rounded = MIN((size_t)zio_roundup_alloc_size(spa, psize), lsize);
    ...
    if (rounded != psize) {
      ...
      psize = rounded;
      zio_push_transform(zio, cdata, psize, rounded, NULL);
    }
  } else {
    ASSERT3U(psize, !=, 0);
  }
   ...
   */
  if (!BP_IS_HOLE(bp) && BP_GET_LOGICAL_BIRTH(bp) == zio->io_txg &&
      BP_GET_PSIZE(bp) == psize && pass >= zfs_sync_pass_rewrite) {
    VERIFY3U(psize, !=, 0);
  ...
  if (psize == 0) {
    ...
    if (zp->zp_dedup) {
      ...
      ASSERT(!zp->zp_encrypt || DMU_OT_IS_ENCRYPTED(zp->zp_type));
...
static zio_t *zio_free_bp_init(zio_t *zio) {
  blkptr_t *bp = zio->io_bp;
...
static void zio_taskq_dispatch(zio_t *zio, zio_taskq_type_t q,
                               boolean_t cutinline) {
  spa_t *spa = zio->io_spa;
...
static boolean_t zio_taskq_member(zio_t *zio, zio_taskq_type_t q) {
  spa_t *spa = zio->io_spa;
...
static zio_t *zio_issue_async(zio_t *zio) {
...
void zio_delay_interrupt(zio_t *zio) {
  ...
  if (zio->io_target_timestamp != 0) {
    ...
    if (now >= zio->io_target_timestamp) {
      ...
      DTRACE_PROBE2(zio__delay__miss, zio_t *, zio, hrtime_t, now);
      ...
      DTRACE_PROBE3(zio__delay__hit, zio_t *, zio, hrtime_t, now, hrtime_t,
                    diff);

      tid = taskq_dispatch_delay(system_taskq, zio_interrupt, zio, TQ_NOSLEEP,
                                 expire_at_tick);
...
static void zio_deadman_impl(zio_t *pio, int ziodepth) {
  zio_t *cio, *cio_next;
  zio_link_t *zl = NULL;
  vdev_t *vd = pio->io_vd;
  ...
  if (zio_deadman_log_all || (vd != NULL && vd->vdev_ops->vdev_op_leaf)) {
    ...
    zfs_dbgmsg("slow zio[%d]: zio=%px timestamp=%llu "
               ...
               "offset=%llu size=%llu "
               "error=%d",
               ziodepth, pio, pio->io_timestamp, (u_longlong_t)delta,
               pio->io_delta, pio->io_delay, vd ? vd->vdev_path : "NULL",
               vq ? vq->vq_io_complete_ts : 0, pio->io_type, pio->io_priority,
               (u_longlong_t)pio->io_flags, pio->io_stage, pio->io_pipeline,
               pio->io_pipeline_trace, (u_longlong_t)zb->zb_objset,
               (u_longlong_t)zb->zb_object, (u_longlong_t)zb->zb_level,
               (u_longlong_t)zb->zb_blkid, (u_longlong_t)pio->io_offset,
               (u_longlong_t)pio->io_size, pio->io_error);
    (void)zfs_ereport_post(FM_EREPORT_ZFS_DEADMAN, pio->io_spa, vd, zb, pio, 0);
 ...
 */
void zio_deadman(zio_t *pio, const char *tag) {
 ...
 */
static boolean_t zio_execute_stack_check(zio_t *zio) {
...
__attribute__((always_inline)) static inline void __zio_execute(zio_t *zio) {
...
int zio_wait(zio_t *zio) {
  ...
  while (zio->io_executor != NULL) {
    error =
        cv_timedwait_io(&zio->io_cv, &zio->io_lock, ddi_get_lbolt() + timeout);
...
static void zio_reexecute(void *arg) {
  zio_t *pio = arg;
  ...
  while ((gio = zio_walk_parents(pio, &zl)) != NULL) {
    ...
    for (int w = 0; w < ZIO_WAIT_TYPES; w++) {
      gio->io_children[pio->io_child_type][w] += !pio->io_state[w];
    }
...
void zio_suspend(spa_t *spa, zio_t *zio, zio_suspend_reason_t reason) {
  ...
  (void)zfs_ereport_post(FM_EREPORT_ZFS_IO_FAILURE, spa, NULL, NULL, NULL, 0);

  mutex_enter(&spa->spa_suspend_lock);

  if (spa->spa_suspend_zio_root == NULL)
    spa->spa_suspend_zio_root =
        zio_root(spa, NULL, NULL,
...
static void zio_gang_issue_func_done(zio_t *zio) { abd_free(zio->io_abd); }

static zio_t *zio_read_gang(zio_t *pio, blkptr_t *bp, zio_gang_node_t *gn,
                            abd_t *data, uint64_t offset) {
  if (gn != NULL)
    return (pio);
...
static zio_t *zio_rewrite_gang(zio_t *pio, blkptr_t *bp, zio_gang_node_t *gn,
                               abd_t *data, uint64_t offset) {
  zio_t *zio;

  if (gn != NULL) {
    abd_t *gbh_abd = abd_get_from_buf(gn->gn_gbh, SPA_GANGBLOCKSIZE);
    zio = zio_rewrite(pio, pio->io_spa, pio->io_txg, bp, gbh_abd,
                      SPA_GANGBLOCKSIZE, zio_gang_issue_func_done, NULL,
                      pio->io_priority, ZIO_GANG_CHILD_FLAGS(pio),
                      &pio->io_bookmark);
    /*
    ...
    if (gn != pio->io_gang_leader->io_gang_tree) {
      ...
      zio_checksum_compute(zio, BP_GET_CHECKSUM(bp), buf, BP_GET_PSIZE(bp));
...
static zio_t *zio_free_gang(zio_t *pio, blkptr_t *bp, zio_gang_node_t *gn,
                            abd_t *data, uint64_t offset) {
  ...
  if (zio == NULL) {
    zio =
        zio_null(pio, pio->io_spa, NULL, NULL, NULL, ZIO_GANG_CHILD_FLAGS(pio));
  }
  return (zio);
}

static zio_t *zio_claim_gang(zio_t *pio, blkptr_t *bp, zio_gang_node_t *gn,
                             abd_t *data, uint64_t offset) {
  (void)gn, (void)data, (void)offset;
  return (zio_claim(pio, pio->io_spa, pio->io_txg, bp, NULL, NULL,
                    ZIO_GANG_CHILD_FLAGS(pio)));
}

static zio_gang_issue_func_t *zio_gang_issue_func[ZIO_TYPES] = {
    NULL, zio_read_gang, zio_rewrite_gang, zio_free_gang, zio_claim_gang, NULL};
...
static void zio_gang_tree_assemble(zio_t *gio, blkptr_t *bp,
...
static void zio_gang_tree_issue(zio_t *pio, zio_gang_node_t *gn, blkptr_t *bp,
                                abd_t *data, uint64_t offset) {
  ...
  if (gn != NULL) {
    ...
    for (int g = 0; g < SPA_GBH_NBLKPTRS; g++) {
      ...
      if (BP_IS_HOLE(gbp))
        continue;
      zio_gang_tree_issue(zio, gn->gn_child[g], gbp, data, offset);
...
static zio_t *zio_gang_assemble(zio_t *zio) {
  blkptr_t *bp = zio->io_bp;
...
static zio_t *zio_gang_issue(zio_t *zio) {
  blkptr_t *bp = zio->io_bp;
  ...
  if (zio->io_child_error[ZIO_CHILD_GANG] == 0)
    zio_gang_tree_issue(zio, zio->io_gang_tree, bp, zio->io_abd, 0);
  else
...
static void zio_gang_inherit_allocator(zio_t *pio, zio_t *cio) {
...
static void zio_write_gang_done(zio_t *zio) {
  /*
...
static zio_t *zio_write_gang_block(zio_t *pio, metaslab_class_t *mc) {
  spa_t *spa = pio->io_spa;
  ...
  if (pio->io_flags & ZIO_FLAG_IO_ALLOCATING) {
    ...
    VERIFY(zfs_refcount_held(
        &mc->mc_allocator[pio->io_allocator].mca_alloc_slots, pio));
  ...
  error = metaslab_alloc(spa, mc, SPA_GANGBLOCKSIZE, bp, gbh_copies, txg,
                         pio == gio ? NULL : gio->io_bp, flags,
  ...
  if (error) {
    if (pio->io_flags & ZIO_FLAG_IO_ALLOCATING) {
       ...
       * stage.
       */
      metaslab_class_throttle_unreserve(mc, gbh_copies - copies,
  ...
  for (int g = 0; resid != 0; resid -= lsize, g++) {
    lsize = P2ROUNDUP(resid / (SPA_GBH_NBLKPTRS - g), SPA_MINBLOCKSIZE);
    ...
    zio_t *cio = zio_write(
        zio, spa, txg, &gbh->zg_blkptr[g],
        has_data ? abd_get_offset(pio->io_abd, pio->io_size - resid) : NULL,
    ...
    if (pio->io_flags & ZIO_FLAG_IO_ALLOCATING) {
       ...
       * slot for them here.
       */
      VERIFY(metaslab_class_throttle_reserve(mc, zp.zp_copies,
                                             cio->io_allocator, cio, flags));
    }
    zio_nowait(cio);
  }
 ...
 */
static zio_t *zio_nop_write(zio_t *zio) {
  blkptr_t *bp = zio->io_bp;
  ...
  if (ZIO_CHECKSUM_EQUAL(bp->blk_cksum, bp_orig->blk_cksum)) {
    ...
    for (int d = 0; d < BP_GET_NDVAS(bp_orig); d++) {
      vdev_t *tvd =
          vdev_lookup_top(zio->io_spa, DVA_GET_VDEV(&bp_orig->blk_dva[d]));
 ...
 * ==========================================================================
 */
static zio_t *zio_brt_free(zio_t *zio) {
  blkptr_t *bp;

  bp = zio->io_bp;

  if (BP_GET_LEVEL(bp) > 0 || BP_IS_METADATA(bp) ||
...
static zio_t *zio_ddt_read_start(zio_t *zio) {
  blkptr_t *bp = zio->io_bp;
  ...
  if (zio->io_child_error[ZIO_CHILD_DDT]) {
    ...
    for (int p = 0; p < DDT_NPHYS(ddt); p++) {
      ...
      ddt_bp_create(ddt->ddt_checksum, &dde->dde_key, ddp, v, &blk);
      zio_nowait(zio_read(zio, zio->io_spa, &blk,
                          abd_alloc_for_io(zio->io_size, B_TRUE), zio->io_size,
  ...
  zio_nowait(zio_read(zio, zio->io_spa, bp, zio->io_abd, zio->io_size, NULL,
                      NULL, zio->io_priority, ZIO_DDT_CHILD_FLAGS(zio),
                      &zio->io_bookmark));

  return (zio);
}

static zio_t *zio_ddt_read_done(zio_t *zio) {
  blkptr_t *bp = zio->io_bp;
  ...
  if (zio->io_child_error[ZIO_CHILD_DDT]) {
    ...
    if (dde->dde_io->dde_repair_abd != NULL) {
      abd_copy(zio->io_abd, dde->dde_io->dde_repair_abd, zio->io_size);
...
static boolean_t zio_ddt_collision(zio_t *zio, ddt_t *ddt, ddt_entry_t *dde) {
  ...
  for (int p = 0; p < DDT_NPHYS(ddt); p++) {
    ...
    if (phys_birth != 0 && do_raw) {
      ...
      error = zio_wait(zio_read(
          NULL, spa, &blk, tmpabd, psize, NULL, NULL, ZIO_PRIORITY_SYNC_READ,
          ZIO_FLAG_CANFAIL | ZIO_FLAG_SPECULATIVE | ZIO_FLAG_RAW,
          &zio->io_bookmark));
      ...
      error = arc_read(
          NULL, spa, &blk, arc_getbuf_func, &abuf, ZIO_PRIORITY_SYNC_READ,
          ZIO_FLAG_CANFAIL | ZIO_FLAG_SPECULATIVE, &aflags, &zio->io_bookmark);

      if (error == 0) {
        if (abd_cmp_buf(zio->io_orig_abd, abuf->b_data, zio->io_orig_size) != 0)
          error = SET_ERROR(ENOENT);
...
static zio_t *zio_ddt_write(zio_t *zio) {
  spa_t *spa = zio->io_spa;
  blkptr_t *bp = zio->io_bp;
  ...
  if (dde->dde_io == NULL || dde->dde_io->dde_lead_zio[p] == NULL) {
     ...
     */
    zio_link_t *zl = NULL;
    zio_t *pio = zio_walk_parents(dde->dde_io->dde_lead_zio[p], &zl);
    ASSERT(pio);
  ...
  zio_t *cio = zio_write(zio, spa, txg, bp, zio->io_orig_abd, zio->io_orig_size,
                         zio->io_orig_size, &czp, zio_ddt_child_write_ready,
                         NULL, zio_ddt_child_write_done, dde, zio->io_priority,
...
static zio_t *zio_ddt_free(zio_t *zio) {
  spa_t *spa = zio->io_spa;
  blkptr_t *bp = zio->io_bp;
  ddt_t *ddt = ddt_select(spa, bp);
...
static zio_t *zio_io_to_allocate(spa_t *spa, int allocator) {
  zio_t *zio;
  ...
  if (!metaslab_class_throttle_reserve(
          zio->io_metaslab_class, zio->io_prop.zp_copies, allocator, zio, 0)) {
    return (NULL);
  }
...
static zio_t *zio_dva_throttle(zio_t *zio) {
  spa_t *spa = zio->io_spa;
  zio_t *nio;
  metaslab_class_t *mc;
  ...
  if (zio->io_priority == ZIO_PRIORITY_SYNC_WRITE ||
      !mc->mc_alloc_throttle_enabled || zio->io_child_type == ZIO_CHILD_GANG ||
...
static void zio_allocate_dispatch(spa_t *spa, int allocator) {
  zio_t *zio;
...
static zio_t *zio_dva_allocate(zio_t *zio) {
  spa_t *spa = zio->io_spa;
  metaslab_class_t *mc;
  blkptr_t *bp = zio->io_bp;
  int error;
   ...
   */
  ASSERT(ZIO_HAS_ALLOCATOR(zio));
  error = metaslab_alloc(spa, mc, zio->io_size, bp, zio->io_prop.zp_copies,
                         zio->io_txg, NULL, flags, &zio->io_alloc_list, zio,
                         zio->io_allocator);
  ...
  if (error == ENOSPC && mc != spa_normal_class(spa)) {
    ...
    if ((mc == spa_dedup_class(spa) ||
         (spa_special_has_ddt(spa) && mc == spa_special_class(spa))) &&
      ...
      zfs_dbgmsg("%s[%d]: %s class spilling, req size %d, "
                 ...
                 (int)zio->io_size, (u_longlong_t)metaslab_class_get_alloc(mc),
    ...
    if (mc->mc_alloc_throttle_enabled &&
      ...
      metaslab_class_throttle_unreserve(mc, zio->io_prop.zp_copies,
                                        zio->io_allocator, zio);
      zio->io_flags &= ~ZIO_FLAG_IO_ALLOCATING;

      VERIFY(metaslab_class_throttle_reserve(
          spa_normal_class(spa), zio->io_prop.zp_copies, zio->io_allocator, zio,
    ...
    if (zfs_flags & ZFS_DEBUG_METASLAB_ALLOC) {
      zfs_dbgmsg("%s: metaslab allocation failure, "
                 ...
                 spa_name(spa), zio, (u_longlong_t)zio->io_size, error);
    }
  ...
  if (error != 0) {
    if (error != ENOSPC || (zfs_flags & ZFS_DEBUG_METASLAB_ALLOC)) {
...
static zio_t *zio_dva_free(zio_t *zio) {
  metaslab_free(zio->io_spa, zio->io_bp, zio->io_txg, B_FALSE);

  return (zio);
}

static zio_t *zio_dva_claim(zio_t *zio) {
  int error;
...
static void zio_dva_unallocate(zio_t *zio, zio_gang_node_t *gn, blkptr_t *bp) {
  ...
  if (!BP_IS_HOLE(bp)) {
    metaslab_free(zio->io_spa, bp, BP_GET_LOGICAL_BIRTH(bp), B_TRUE);
  }

  if (gn != NULL) {
    for (int g = 0; g < SPA_GBH_NBLKPTRS; g++) {
      zio_dva_unallocate(zio, gn->gn_child[g], &gn->gn_gbh->zg_blkptr[g]);
    }
  }
}
...
int zio_alloc_zil(spa_t *spa, objset_t *os, uint64_t txg, blkptr_t *new_bp,
  ...
  int allocator =
      (uint_t)cityhash1(os->os_dsl_dataset->ds_object) % spa->spa_alloc_count;
  ZIOSTAT_BUMP(ziostat_total_allocations);
  error = metaslab_alloc(spa, spa_log_class(spa), size, new_bp, 1, txg, NULL,
                         flags, &io_alloc_list, NULL, allocator);
  *slog = (error == 0);
  if (error != 0) {
    error = metaslab_alloc(spa, spa_embedded_log_class(spa), size, new_bp, 1,
                           txg, NULL, flags, &io_alloc_list, NULL, allocator);
  }
  if (error != 0) {
    ZIOSTAT_BUMP(ziostat_alloc_class_fallbacks);
    error = metaslab_alloc(spa, spa_normal_class(spa), size, new_bp, 1, txg,
                           NULL, flags, &io_alloc_list, NULL, allocator);
  }
  ...
  if (error == 0) {
    ...
    BP_SET_CHECKSUM(new_bp, spa_version(spa) >= SPA_VERSION_SLIM_ZIL
                                ? ZIO_CHECKSUM_ZILOG2
                                : ZIO_CHECKSUM_ZILOG);
    if (spa_version(spa) < SPA_VERSION_SLIM_ZIL) {
      zfs_dbgmsg("%s with spa_version(spa)=%ld and SPA_VERSION_SLIM_ZIL=%lld: "
                 "using old ZIL checksum %lld ZIO_CHECKSUM_ZILOG\n",
                 spa_name(spa), spa_version(spa), SPA_VERSION_SLIM_ZIL,
                 BP_GET_CHECKSUM(new_bp));
    } else {
      zfs_dbgmsg("%s with spa_version(spa)=%ld and SPA_VERSION_SLIM_ZIL=%lld: "
                 "using new ZIL checksum %lld ZIO_CHECKSUM_ZILOG2\n",
                 spa_name(spa), spa_version(spa), SPA_VERSION_SLIM_ZIL,
                 BP_GET_CHECKSUM(new_bp));
    }
    BP_SET_TYPE(new_bp, DMU_OT_INTENT_LOG);
    ...
    if (os->os_encrypted) {
      ...
      VERIFY0(spa_crypt_get_salt(spa, dmu_objset_id(os), salt));
 ...
 */
static zio_t *zio_vdev_io_start(zio_t *zio) {
  vdev_t *vd = zio->io_vd;
  uint64_t align;
  spa_t *spa = zio->io_spa;
  ...
  if (zio->io_type == ZIO_TYPE_WRITE) {
    ...
    if (zio->io_vd->vdev_noalloc) {
      ASSERT(zio->io_flags & (ZIO_FLAG_PHYSICAL | ZIO_FLAG_SELF_HEAL |
...
static zio_t *zio_vdev_io_done(zio_t *zio) {
  vdev_t *vd = zio->io_vd;
  ...
  ASSERT(zio->io_type == ZIO_TYPE_READ || zio->io_type == ZIO_TYPE_WRITE ||
         zio->io_type == ZIO_TYPE_FLUSH || zio->io_type == ZIO_TYPE_TRIM);
  ...
  if (vd != NULL && vd->vdev_ops->vdev_op_leaf &&
    ...
    if (zio_injection_enabled && zio->io_error == 0)
      zio->io_error = zio_handle_device_injections(vd, zio, EIO, EILSEQ);
...
static zio_t *zio_vdev_io_assess(zio_t *zio) {
  vdev_t *vd = zio->io_vd;
  ...
  if (zio->io_error && vd == NULL &&
    ...
    zio_taskq_dispatch(zio, ZIO_TASKQ_ISSUE, zio_requeue_io_start_cut_in_line);
    return (NULL);
  }
   ...
   */
  if (zio->io_error == ENXIO && zio->io_type == ZIO_TYPE_WRITE && vd != NULL &&
 ...
 */
static zio_t *zio_encrypt(zio_t *zio) {
  zio_prop_t *zp = &zio->io_prop;
  spa_t *spa = zio->io_spa;
  blkptr_t *bp = zio->io_bp;
  ...
  if (zio->io_flags & ZIO_FLAG_RAW_ENCRYPT) {
    ...
    if (zp->zp_byteorder != ZFS_HOST_BYTEORDER && ot == DMU_OT_DNODE) {
      ...
      dmu_ot_byteswap[DMU_OT_BYTESWAP(ot)].ob_func(bswap_buf, psize);
  ...
  if (BP_GET_LEVEL(bp) > 0) {
    BP_SET_CRYPT(bp, B_TRUE);
    VERIFY0(zio_crypt_do_indirect_mac_checksum_abd(
        B_TRUE, zio->io_orig_abd, BP_GET_LSIZE(bp), BP_SHOULD_BYTESWAP(bp),
        mac));
  ...
  if (ot == DMU_OT_OBJSET) {
    ...
    ASSERT3U(BP_GET_COMPRESS(bp), ==, ZIO_COMPRESS_OFF);
    BP_SET_CRYPT(bp, B_TRUE);
    VERIFY0(spa_do_crypt_objset_mac_abd(B_TRUE, spa, dsobj, zio->io_abd, psize,
                                        BP_SHOULD_BYTESWAP(bp)));
    return (zio);
  }
  ...
  if (!DMU_OT_IS_ENCRYPTED(ot)) {
    BP_SET_CRYPT(bp, B_TRUE);
    VERIFY0(spa_do_crypt_mac_abd(B_TRUE, spa, dsobj, zio->io_abd, psize, mac));
    zio_crypt_encode_mac_bp(bp, mac);
    return (zio);
  }
  ...
  VERIFY0(spa_do_crypt_abd(B_TRUE, spa, &zio->io_bookmark, BP_GET_TYPE(bp),
                           BP_GET_DEDUP(bp), BP_SHOULD_BYTESWAP(bp), salt, iv,
                           mac, psize, zio->io_abd, eabd, &no_crypt));
...
static zio_t *zio_checksum_verify(zio_t *zio) {
  ...
  if ((error = zio_checksum_error(zio, &info)) != 0) {
    zio->io_error = error;
    if (error == ECKSUM && !(zio->io_flags & ZIO_FLAG_SPECULATIVE)) {
      if (zio->io_flags & ZIO_FLAG_DIO_READ) {
        ...
        (void)zfs_ereport_start_checksum(zio->io_spa, zio->io_vd,
                                         &zio->io_bookmark, zio, zio->io_offset,
                                         zio->io_size, &info);
      }
    }
  }
...
static zio_t *zio_dio_checksum_verify(zio_t *zio) {
  zio_t *pio = zio_unique_parent(zio);
  int error;
 ...
 */
void zio_dio_chksum_verify_error_report(zio_t *zio) {
  ...
  if (zio->io_type == ZIO_TYPE_WRITE) {
     ...
     */
    (void)zfs_ereport_post(FM_EREPORT_ZFS_DIO_VERIFY_WR, zio->io_spa,
                           zio->io_vd, &zio->io_bookmark, zio, 0);
  } else {
    /*
     * Report dio_verify_rd ZED event.
     */
    (void)zfs_ereport_post(FM_EREPORT_ZFS_DIO_VERIFY_RD, zio->io_spa,
                           zio->io_vd, &zio->io_bookmark, zio, 0);
  }
}
 ...
 * ==========================================================================
 */
static zio_t *zio_ready(zio_t *zio) {
  blkptr_t *bp = zio->io_bp;
  zio_t *pio, *pio_next;
  zio_link_t *zl = NULL;

  if (zio_wait_for_children(
          zio, ZIO_CHILD_LOGICAL_BIT | ZIO_CHILD_GANG_BIT | ZIO_CHILD_DDT_BIT,
          ZIO_WAIT_READY)) {
    return (NULL);
  }
  ...
  if (zio->io_error != 0) {
    ...
    if (zio->io_flags & ZIO_FLAG_IO_ALLOCATING) {
       ...
       */
      metaslab_class_throttle_unreserve(zio->io_metaslab_class,
  ...
  if (zio_injection_enabled && zio->io_spa->spa_syncing_txg == zio->io_txg)
 ...
 */
static void zio_dva_throttle_done(zio_t *zio) {
...
static zio_t *zio_done(zio_t *zio) {
  /*
  ...
  if (zio->io_flags & ZIO_FLAG_IO_ALLOCATING) {
    ...
    VERIFY(zfs_refcount_not_held(
        &zio->io_metaslab_class->mc_allocator[zio->io_allocator]
             .mca_alloc_slots,
        zio));
  }
  ...
  if (zio->io_bp != NULL && !BP_IS_EMBEDDED(zio->io_bp)) {
    ...
    ASSERT(memcmp(zio->io_bp, &zio->io_bp_copy, sizeof(blkptr_t)) == 0 ||
           (zio->io_bp == zio_unique_parent(zio)->io_bp));
    if (zio->io_type == ZIO_TYPE_WRITE && !BP_IS_HOLE(zio->io_bp) &&
        zio->io_bp_override == NULL && !(zio->io_flags & ZIO_FLAG_IO_REPAIR)) {
      ASSERT3U(zio->io_prop.zp_copies, <=, BP_GET_NDVAS(zio->io_bp));
  ...
  if (zio->io_delay >= MSEC2NSEC(zio_slow_io_ms)) {
    if (zio->io_vd != NULL && !vdev_is_dead(zio->io_vd)) {
       ...
       * of it.
       */
      if (zfs_ereport_is_valid(FM_EREPORT_ZFS_DELAY, zio->io_spa, zio->io_vd,
                               zio)) {
        mutex_enter(&zio->io_vd->vdev_stat_lock);
        zio->io_vd->vdev_stat.vs_slow_ios++;
        mutex_exit(&zio->io_vd->vdev_stat_lock);

        (void)zfs_ereport_post(FM_EREPORT_ZFS_DELAY, zio->io_spa, zio->io_vd,
                               &zio->io_bookmark, zio, 0);
      }
    }
  }
  ...
  if (zio->io_error) {
    ...
    if (zio->io_error != ECKSUM && zio->io_vd != NULL &&
        ...
        !(zio->io_flags & ZIO_FLAG_DIO_CHKSUM_ERR)) {
      int ret = zfs_ereport_post(FM_EREPORT_ZFS_IO, zio->io_spa, zio->io_vd,
                                 &zio->io_bookmark, zio, 0);
      if (ret != EALREADY) {
        mutex_enter(&zio->io_vd->vdev_stat_lock);
        if (zio->io_type == ZIO_TYPE_READ)
    ...
    if ((zio->io_error == EIO ||
         !(zio->io_flags & (ZIO_FLAG_SPECULATIVE | ZIO_FLAG_DONT_PROPAGATE))) &&
        !(zio->io_flags & ZIO_FLAG_DIO_CHKSUM_ERR) && zio == zio->io_logical) {
      /*
      ...
      (void)zfs_ereport_post(FM_EREPORT_ZFS_DATA, zio->io_spa, NULL,
                             &zio->io_bookmark, zio, 0);
    }
  }
  ...
  if (zio->io_error && zio == zio->io_logical) {
    ...
    if (IO_IS_ALLOCATING(zio) && !(zio->io_flags & ZIO_FLAG_CANFAIL) &&
        !(zio->io_flags & ZIO_FLAG_DIO_CHKSUM_ERR)) {
    ...
    if ((zio->io_type == ZIO_TYPE_READ || zio->io_type == ZIO_TYPE_FREE) &&
        !(zio->io_flags & ZIO_FLAG_SCAN_THREAD) && zio->io_error == ENXIO &&
  ...
  if (zio->io_reexecute) {
    ...
    if ((pio = zio_unique_parent(zio)) != NULL) {
       ...
       */
      spa_taskq_dispatch(zio->io_spa, ZIO_TYPE_CLAIM, ZIO_TASKQ_ISSUE,
 ...
 * ==========================================================================
 */
static zio_pipe_stage_t *zio_pipeline[] = {NULL,
                                           zio_read_bp_init,
                                           zio_write_bp_init,
                                           zio_free_bp_init,
                                           zio_issue_async,
...
int zbookmark_compare(uint16_t dbss1, uint8_t ibs1, uint16_t dbss2,
                      uint8_t ibs2, const zbookmark_phys_t *zb1,
  ...
  if (zb1->zb_object == zb2->zb_object && zb1->zb_level == zb2->zb_level &&
 ...
 *  visited.
 */
boolean_t zbookmark_subtree_completed(const dnode_phys_t *dnp,
                                      const zbookmark_phys_t *subtree_root,
                                      const zbookmark_phys_t *last_block) {
  ...
  return (zbookmark_compare(dnp->dn_datablkszsec, dnp->dn_indblkshift,
                            1ULL << (DNODE_BLOCK_SHIFT - SPA_MINBLOCKSHIFT), 0,
                            &mod_zb, last_block) <= 0);
}
 ...
 */
boolean_t zbookmark_subtree_tbd(const dnode_phys_t *dnp,
                                const zbookmark_phys_t *subtree_root,
                                const zbookmark_phys_t *last_block) {
  ASSERT0(last_block->zb_level);
  if (dnp == NULL)
...
ZFS_MODULE_PARAM(
    zfs_zio, zio_, slow_io_ms, INT, ZMOD_RW,
