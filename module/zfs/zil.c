...
int zil_parse(zilog_t *zilog, zil_parse_blk_func_t *parse_blk_func,
  ...
  if (BP_IS_HOLE(&zh->zh_log)) {
    ...
    zfs_dbgmsg(" zil_parse: starting blk %llu, claim_blk_seq %llu, "
               "claim_lr_seq %llu\n",
  ...
  for (blk = zh->zh_log; !BP_IS_HOLE(&blk); blk = next_blk) {
    ...
    if (first_block && remount) {
      ...
      if (starting_blk_cmt != NULL && final_blk_cmt != NULL) {
        zc_eck first_val =
            starting_blk_cmt->blk_digest; // get_hash(&cksum_map,
                                          // &(starting_blk_cmt->blk_num));
        zc_eck tail_hash =
            final_blk_cmt->blk_digest; // get_hash(&cksum_map,
                                       // &(final_blk_cmt->blk_num));
        final_blk_cmt->blk_digest = tail_hash;
        zfs_dbgmsg(" zil_parse: starting blk commitment (blk_num=%llu) hash: "
                   "%016llx:%016llx:%016llx:%016llx\n",
                   (u_longlong_t)starting_blk_cmt->blk_num.zc_word[ZIL_ZC_SEQ],
                   (u_longlong_t)first_val.zc_word[0],
                   (u_longlong_t)first_val.zc_word[1],
                   (u_longlong_t)first_val.zc_word[2],
                   (u_longlong_t)first_val.zc_word[3]);
        zfs_dbgmsg(" zil_parse: final blk commitment (blk_num=%llu) hash: "
                   "%016llx:%016llx:%016llx:%016llx\n",
                   (u_longlong_t)final_blk_cmt->blk_num.zc_word[ZIL_ZC_SEQ],
                   (u_longlong_t)tail_hash.zc_word[0],
                   (u_longlong_t)tail_hash.zc_word[1],
                   (u_longlong_t)tail_hash.zc_word[2],
                   (u_longlong_t)tail_hash.zc_word[3]);
...
static void zil_lwb_flush_vdevs_done(zio_t *zio) {
  ...
  zfs_dbgmsg(
      " zilog->zl_commit_lr_seq=%llu, zilog->zl_lr_seq=%llu, lwb->lwb_issued_txg=%llu, \
		lwb->lwb_alloc_txg=%llu, lwb->lwb_max_txg=%llu\n",
      (u_longlong_t)zilog->zl_commit_lr_seq, (u_longlong_t)zilog->zl_lr_seq,
  ...
  zfs_dbgmsg(" name=%s\t lwb_blk->cksum_seq_no=%llu in txg_sync=%llu w/ "
             "DVA=<%llu:%llx:%llx>\n",
             name, (u_longlong_t)lwb->lwb_blk.blk_cksum.zc_word[ZIL_ZC_SEQ],
             (u_longlong_t)lwb->lwb_issued_txg,
  ...
  dump_zil_commitment2(&zil_tail_commitment);
  zfs_dbgmsg(
      " name=%s\t zio=%p lwb_blk->cksum_seq_no=%016llx:%016llx:%016llx:%016llx in txg_sync=%llu w/ zio->io_cksum=%016llx:%016llx:%016llx:%016llx\
			lwb->io_cksum=%016llx:%016llx:%016llx:%016llx\n",
      name, (void *)zio, (u_longlong_t)lwb->lwb_blk.blk_cksum.zc_word[0],
      ...
      (u_longlong_t)lwb->lwb_issued_txg, (u_longlong_t)zio->io_cksum.zc_word[0],
...
static void zil_process_commit_list(zilog_t *zilog, zil_commit_waiter_t *zcw,
  ...
  while ((itx = list_remove_head(&zilog->zl_itx_commit_list)) != NULL) {
    ...
    if (frozen || !synced || lrc->lrc_txtype == TX_COMMIT) {
      if (frozen)
        zfs_dbgmsg(
            " is pool frozen?(%d), if yes, we persist everything to the ZIL\n",
            (int)frozen);
      if (lwb != NULL) {
