...
[[maybe_unused]] static int zil_prt_rec_write_cb(void *data, size_t len,
                                                 void *unused) {
  (void)data;
  (void)len;
  (void)unused;
...
[[maybe_unused]] static void print_zilc_zc_eck(const zil_chain_t *chain) {
  zfs_dbgmsg(" chain->zc_eck.zec_cksum=%016llx:%016llx:%016llx:%016llx\n",
             (u_longlong_t)chain->zc_eck.zec_cksum.zc_word[0],
             (u_longlong_t)chain->zc_eck.zec_cksum.zc_word[1],
             (u_longlong_t)chain->zc_eck.zec_cksum.zc_word[2],
             (u_longlong_t)chain->zc_eck.zec_cksum.zc_word[3]);
}

[[maybe_unused]] static void print_blk(const blkptr_t *bp) {
  char type[256];
  char buf[500];
  ...
  if (bp != NULL) {
    if (BP_GET_TYPE(bp) & DMU_OT_NEWTYPE) {
      dmu_object_byteswap_t bswap = DMU_OT_BYTESWAP(BP_GET_TYPE(bp));
      (void)snprintf(type, sizeof(type), "bswap %s %s",
                     DMU_OT_IS_METADATA(BP_GET_TYPE(bp)) ? "metadata" : "data",
                     dmu_ot_byteswap[bswap].ob_name);
    } else {
      (void)strlcpy(type, dmu_ot[BP_GET_TYPE(bp)].ot_name, sizeof(type));
    }
  ...
  SNPRINTF_BLKPTR(kmem_scnprintf, ' ', buf, 500, bp, type, checksum, compress);
  zfs_dbgmsg("%s\n", buf);
}

[[maybe_unused]] static void print_zil_header(zilog_t *zilog) {
  ...
  zfs_dbgmsg(" zilog=%p with on-disk log record sequence number %llu, \
		last committed on-disk lr seq %llu, txg of last zil_destroy is %llu, \
		current replay seq number %llu\n",
             (void *)zilog, (u_longlong_t)zilog->zl_lr_seq,
             (u_longlong_t)zilog->zl_commit_lr_seq,
             (u_longlong_t)zilog->zl_destroy_txg,
             (u_longlong_t)zilog->zl_replaying_seq);
  zfs_dbgmsg(" zil header %p: claim_txg %llu,"
             "claim_blk_seq %llu, claim_lr_seq %llu",
             (void *)(zh), (u_longlong_t)zh->zh_claim_txg,
...
__attribute__((unused)) static void zfs_dbgmsg_log_block(zilog_t *zilog) {
  ...
  zfs_dbgmsg(" zilog=%p: on-disk log record sequence number %llu, \
		last committed on-disk lr seq %llu, txg of last zil_destroy %llu,\
		 current replay seq number %llu\n",
             (void *)zilog, (u_longlong_t)zilog->zl_lr_seq,
             (u_longlong_t)zilog->zl_commit_lr_seq,
             (u_longlong_t)zilog->zl_destroy_txg,
...
static int zil_clear_log_block(zilog_t *zilog, const blkptr_t *bp, void *tx,
                               uint64_t first_txg);
static int zil_incr_blks(zilog_t *zilog, const blkptr_t *bp, void *arg,
                         uint64_t claim_txg);
static int zil_free_log_block(zilog_t *zilog, const blkptr_t *bp, void *tx,
                              uint64_t claim_txg);
static int zil_claim_log_block(zilog_t *zilog, const blkptr_t *bp, void *tx,
                               uint64_t first_txg);
...
static void zil_bp_tree_init(zilog_t *zilog) {
  avl_create(&zilog->zl_bp_tree, zil_bp_compare, sizeof(zil_bp_node_t),
             offsetof(zil_bp_node_t, zn_node));
}
...
static int zil_kstats_global_update(kstat_t *ksp, int rw) {
 ...
 */
static int zil_read_log_block(zilog_t *zilog, boolean_t decrypt,
                              const blkptr_t *bp, blkptr_t *nbp, char **begin,
                              char **end, arc_buf_t **abuf) {
  ...
  SET_BOOKMARK(&zb, bp->blk_cksum.zc_word[ZIL_ZC_OBJSET], ZB_ZIL_OBJECT,
               ZB_ZIL_LEVEL, bp->blk_cksum.zc_word[ZIL_ZC_SEQ]);
  ...
  if (error == 0) {
    ...
    if (BP_GET_CHECKSUM(bp) == ZIO_CHECKSUM_ZILOG2) {
      ...
      if (memcmp(&cksum, &zilc->zc_next_blk.blk_cksum, sizeof(cksum)) ||
          zilc->zc_nused < sizeof(*zilc) || zilc->zc_nused > size) {
      ...
      zfs_dbgmsg(" [ZIO_CHECKSUM_ZILOG2] "
                 "blk_cksum=%016llx:%016llx:%016llx:%016llx\tezk=%016llx:%"
                 "016llx:%016llx:%016llx\n",
                 (u_longlong_t)bp_seqno.zc_word[0],
                 (u_longlong_t)bp_seqno.zc_word[1],
                 (u_longlong_t)bp_seqno.zc_word[2],
                 ((u_longlong_t)bp_seqno.zc_word[3] - 1),
                 (u_longlong_t)eck.zec_cksum.zc_word[0],
                 (u_longlong_t)eck.zec_cksum.zc_word[1],
                 (u_longlong_t)eck.zec_cksum.zc_word[2],
                 (u_longlong_t)eck.zec_cksum.zc_word[3]);
    } else {
      char *lr = (*abuf)->b_data;
      ...
      zio_eck_t eck = zilc->zc_eck;
      zio_cksum_t bp_seqno = zilc->zc_next_blk.blk_cksum;
      zfs_dbgmsg(" [not ZIO_CHECKSUM_ZILOG2] "
                 "blk_cksum=%016llx:%016llx:%016llx:%016llx\tezk=%016llx:%"
                 "016llx:%016llx:%016llx\n",
                 (u_longlong_t)bp_seqno.zc_word[0],
                 (u_longlong_t)bp_seqno.zc_word[1],
...
static int zil_read_log_data(zilog_t *zilog, const lr_write_t *lr, void *wbuf) {
  ...
  SET_BOOKMARK(&zb, dmu_objset_id(zilog->zl_os), lr->lr_foid, ZB_ZIL_LEVEL,
...
void zil_sums_init(zil_sums_t *zs) {
...
void zil_sums_fini(zil_sums_t *zs) {
...
void zil_kstat_values_update(zil_kstat_values_t *zs, zil_sums_t *zil_sums) {
  zs->zil_commit_count.value.ui64 = wmsum_value(&zil_sums->zil_commit_count);
  ...
  zs->zil_itx_count.value.ui64 = wmsum_value(&zil_sums->zil_itx_count);
...
static const char *
print_callback_for_blk_in_zil_parse(zil_parse_blk_func_t *parse_blk_func) {
 ...
 */
int zil_parse(zilog_t *zilog, zil_parse_blk_func_t *parse_blk_func,
  ...
  starting_blk_cmt =
      get_zil_header_cmt_for_dsl(name, ccf_zil_header_commitments);
  ...
  zfs_dbgmsg(
      " remount=%d zh->zh_log is a hole=%d [(!BP_IS_EMBEDDED(bp)=%d && "
      "DVA_IS_EMPTY(BP_IDENTITY(bp))=%d)] (start_blk=%llu, end_blk=%llu)\n",
      remount, BP_IS_HOLE(&(zh->zh_log)), (!BP_IS_EMBEDDED(&(zh->zh_log))),
      DVA_IS_EMPTY(BP_IDENTITY(&(zh->zh_log))),
  ...
  for (blk = zh->zh_log; !BP_IS_HOLE(&blk); blk = next_blk) {
    ...
    if (first_block && remount) {
      ...
      if (starting_blk_cmt != NULL) {
        if (memcmp(blk.blk_cksum.zc_word, starting_blk_cmt->blk_num.zc_word,
                   sizeof(zio_cksum_t)) == 0) {
          ...
          zfs_dbgmsg(" error, zil headers *not* match ..\n\
                     ...
                     (u_longlong_t)blk.blk_cksum.zc_word[0],
                     (u_longlong_t)blk.blk_cksum.zc_word[1],
                     (u_longlong_t)blk.blk_cksum.zc_word[2],
                     (u_longlong_t)blk.blk_cksum.zc_word[3],
    ...
    error =
        zil_read_log_block(zilog, decrypt, &blk, &next_blk, &lrp, &end, &abuf);

    if (remount) {
      if (final_blk_cmt != NULL) {
        if (memcmp(blk.blk_cksum.zc_word, final_blk_cmt->blk_num.zc_word,
                   sizeof(zio_cksum_t)) == 0) {
          zfs_dbgmsg(" this is the tail, error should be 0 (error=%d) ..\n",
                     error);
        } else if (memcmp(blk.blk_cksum.zc_word, final_blk_cmt->blk_num.zc_word,
                          sizeof(zio_cksum_t) -
                              sizeof(blk.blk_cksum.zc_word[ZIL_ZC_SEQ])) == 0) {
          if (blk.blk_cksum.zc_word[ZIL_ZC_SEQ] >
              final_blk_cmt->blk_num.zc_word[ZIL_ZC_SEQ]) {
            zfs_dbgmsg(
                " this block is past tail error should be > 0 (error=%d) ..\n",
                error);
            if (error <= 0) {
    ...
    for (; lrp < end; lrp += reclen) {
      ...
      if (reclen < sizeof(lr_t) || reclen > end - lrp) {
        cmn_err(CE_WARN, "zil_parse: lr_t has an invalid reclen");
        error = SET_ERROR(ECKSUM);
        arc_buf_destroy(abuf, &abuf);
        goto done;
      }
...
static int zil_noop_log_record(zilog_t *zilog, const lr_t *lrc, void *tx,
                               uint64_t first_txg) {
...
static int zil_claim_log_block(zilog_t *zilog, const blkptr_t *bp, void *tx,
  ...
  return (zio_wait(zio_claim(
      NULL, zilog->zl_spa, tx == NULL ? 0 : first_txg, bp, spa_claim_notify,
      NULL, ZIO_FLAG_CANFAIL | ZIO_FLAG_SPECULATIVE | ZIO_FLAG_SCRUB)));
}

static int zil_claim_write(zilog_t *zilog, const lr_t *lrc, void *tx,
                           uint64_t first_txg) {
...
static int zil_claim_clone_range(zilog_t *zilog, const lr_t *lrc, void *tx,
                                 uint64_t first_txg) {
  ...
  ASSERT3U(lrc->lrc_reclen, >=, sizeof(*lr));
  ASSERT3U(lrc->lrc_reclen, >=,
           offsetof(lr_clone_range_t, lr_bps[lr->lr_nbps]));
...
static int zil_claim_log_record(zilog_t *zilog, const lr_t *lrc, void *tx,
                                uint64_t first_txg) {
...
static int zil_free_write(zilog_t *zilog, const lr_t *lrc, void *tx,
                          uint64_t claim_txg) {
  lr_write_t *lr = (lr_write_t *)lrc;
...
static int zil_free_clone_range(zilog_t *zilog, const lr_t *lrc, void *tx) {
  const lr_clone_range_t *lr = (const lr_clone_range_t *)lrc;
  const blkptr_t *bp;
  spa_t *spa;
  uint_t ii;
...
static int zil_free_log_record(zilog_t *zilog, const lr_t *lrc, void *tx,
                               uint64_t claim_txg) {
 ...
 */
static lwb_t *zil_alloc_lwb(zilog_t *zilog, int sz, blkptr_t *bp,
                            boolean_t slog, uint64_t txg, lwb_state_t state) {
  lwb_t *lwb;
  ...
  if (bp) {
    ...
    BP_ZERO(&lwb->lwb_blk);
    lwb->lwb_slim = (spa_version(zilog->zl_spa) >= SPA_VERSION_SLIM_ZIL);
  }
  lwb->lwb_slog = slog;
  lwb->lwb_error = 0;
  if (lwb->lwb_slim) {
...
static void zil_free_lwb(zilog_t *zilog, lwb_t *lwb) {
  ...
  zfs_dbgmsg(" zilog->zl_header->zh_claim_txg=%llu\t\
		zilog->zl_header->zh_claim_lr_seq=%llu\t\
		zilog->zl_header->zh_claim_blk_seq=%llu\t\
		lwb->lwb_issued_txg=%llu\t\
		lwb->lwb_alloc_txg=%llu\n",
             (u_longlong_t)zilog->zl_header->zh_claim_txg,
             (u_longlong_t)zilog->zl_header->zh_claim_lr_seq,
             (u_longlong_t)zilog->zl_header->zh_claim_blk_seq,
 ...
 * state.
 */
static boolean_t __maybe_unused zilog_is_dirty_in_txg(zilog_t *zilog,
                                                      uint64_t txg) {
  dsl_pool_t *dp = zilog->zl_dmu_pool;
 ...
 * zil_commit.
 */
static void zil_commit_activate_saxattr_feature(zilog_t *zilog) {
  dsl_dataset_t *ds = dmu_objset_ds(zilog->zl_os);
  uint64_t txg = 0;
  dmu_tx_t *tx = NULL;
  ...
  if (spa_feature_is_enabled(zilog->zl_spa, SPA_FEATURE_ZILSAXATTR) &&
    ...
    ds->ds_feature_activation[SPA_FEATURE_ZILSAXATTR] = (void *)B_TRUE;
...
static lwb_t *zil_create(zilog_t *zilog) {
  ...
  if (BP_IS_HOLE(&blk) || BP_SHOULD_BYTESWAP(&blk)) {
    ...
    error = zio_alloc_zil(zilog->zl_spa, zilog->zl_os, txg, &blk, ZIL_MIN_BLKSZ,
                          &slog);
    if (error == 0)
 ...
 */
boolean_t zil_destroy(zilog_t *zilog, boolean_t keep_first) {
  const zil_header_t *zh = zilog->zl_header;
  lwb_t *lwb;
  dmu_tx_t *tx;
  uint64_t txg;
...
void zil_destroy_sync(zilog_t *zilog, dmu_tx_t *tx) {
  ...
  (void)zil_parse(zilog, zil_free_log_block, zil_free_log_record, tx,
                  zilog->zl_header->zh_claim_txg, B_FALSE);
}
...
int zil_claim(dsl_pool_t *dp, dsl_dataset_t *ds, void *txarg) {
  ...
  error = dmu_objset_own_obj(dp, ds->ds_object, DMU_OST_ANY, B_FALSE, B_FALSE,
                             FTAG, &os);
  if (error != 0) {
    /*
  ...
  if (spa_get_log_state(zilog->zl_spa) == SPA_LOG_CLEAR ||
      (zilog->zl_spa->spa_uberblock.ub_checkpoint_txg != 0 &&
       zh->zh_claim_txg == 0)) {
    if (!BP_IS_HOLE(&zh->zh_log)) {
      (void)zil_parse(zilog, zil_clear_log_block, zil_noop_log_record, tx,
                      first_txg, B_FALSE);
    }
    BP_ZERO(&zh->zh_log);
    if (os->os_encrypted)
  ...
  if (zh->zh_claim_txg == 0 && !BP_IS_HOLE(&zh->zh_log)) {
    (void)zil_parse(zilog, zil_claim_log_block, zil_claim_log_record, tx,
                    first_txg, B_FALSE);
...
int zil_check_log_chain(dsl_pool_t *dp, dsl_dataset_t *ds, void *tx) {
   ...
   */
  error = zil_parse(
      zilog, zil_claim_log_block, zil_claim_log_record, tx,
      zilog->zl_header->zh_claim_txg ? -1ULL : spa_min_claim_txg(os->os_spa),
      B_FALSE);
 ...
 */
static void zil_commit_waiter_link_nolwb(zil_commit_waiter_t *zcw,
                                         list_t *nolwb) {
  ASSERT(!list_link_active(&zcw->zcw_node));
...
static void zil_lwb_flush_vdevs_done(zio_t *zio) {
...
#if 0
	zfs_dbgmsg(" zilog->zl_commit_lr_seq=%llu, zilog->zl_lr_seq=%llu, lwb->lwb_issued_txg=%llu, \
  ...
  zil_commitment_t *tail_commitment = generate_zil_tail_cmt(
      name, lwb->lwb_issued_txg, lwb->lwb_blk.blk_cksum, lwb->lwb_blk.blk_dva);
  zil_commitment_t *tail_commitment_copy = generate_zil_tail_cmt(
      name, lwb->lwb_issued_txg, lwb->lwb_blk.blk_cksum, lwb->lwb_blk.blk_dva);
  mutex_exit(&my_mutex);
  ...
  zil_tail_commitment = *tail_commitment;
  list_create(&(tail_commitment_copy->waiters), sizeof(ccf_waiter_t),
              offsetof(ccf_waiter_t, zcw_ccf_node));
  list_create(&(zil_tail_commitment.waiters), sizeof(ccf_waiter_t),
              offsetof(ccf_waiter_t, zcw_ccf_node));
  ...
  while ((zcw = list_remove_head(&lwb->lwb_waiters)) != NULL) {
    mutex_enter(&zcw->zcw_lock);
    zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id =
...
// memcpy(zcw_copy, zcw->zcw_ccf_ptr, sizeof(ccf_cond_var_t));
#if 0
		zfs_dbgmsg(" [Step 3: after memcpy] (*zcw_copy)->zcw_block_id=%llu\
    ...
    if (head_copy == NULL) {
      zfs_dbgmsg(
          " [SOS: head_copy is NULL] zil_tail_commitment.waiters is empty\n");
    }
/*
else {
        //head_copy = zcw_copy;
...
*/
...
#endif
#if 0
		zfs_dbgmsg(" > zcw before copy: block id=%llu zcw=%p *zcw_copy=%p zcw_copy=%p\n", (u_longlong_t)zcw->zcw_block_id, (void*)zcw, (void*)(zcw_copy), (void*)(zcw_copy));

   	    list_insert_tail(&(zil_tail_commitment.waiters), zcw_copy);
		zfs_dbgmsg(" > zcw after copy: block id=%llu zcw=%p *zcw_copy=%p zcw_copy=%p\n", (u_longlong_t)zcw->zcw_block_id, (void*)zcw, (void*)(zcw_copy), (void*)(zcw_copy));
		zil_commit_waiter_t* head_copy = list_head(&(zil_tail_commitment.waiters));

		zfs_dbgmsg(" > zcw after copy: block id=%llu zcw=%p *zcw_copy=%p zcw_copy=%p *head_copy=%p head_copy=%p\n", (u_longlong_t)zcw->zcw_block_id, (void*)zcw, (void*)(zcw_copy), (void*)(zcw_copy), (void*)(head_copy), (void*)head_copy);
		//zfs_dbgmsg(" > zcw_copy after copy: block id=%llu\n", (u_longlong_t)((*zcw_copy)->zcw_block_id));
#endif
  ...
  commitments_list_node_t *to_be_inserted =
      alloc_node(sizeof(commitments_list_node_t));
 ...
 */
static void zil_lwb_flush_wait_all(zilog_t *zilog, uint64_t txg) {
  ...
  while (lwb != NULL) {
    if (lwb->lwb_issued_txg <= txg) {
      ...
      IMPLY(lwb->lwb_issued_txg > 0, lwb->lwb_state == LWB_STATE_FLUSH_DONE);
    }
...
static void zil_lwb_set_zio_dependency(zilog_t *zilog, lwb_t *lwb) {
  ...
  if (prev_lwb == NULL || prev_lwb->lwb_state == LWB_STATE_FLUSH_DONE)
    return;
 ...
 */
static uint_t zil_lwb_plan(zilog_t *zilog, uint64_t size, uint_t *minsize) {
 ...
 */
static uint_t zil_lwb_predict(zilog_t *zilog) {
  uint_t m, o;
 ...
 */
static lwb_t *zil_lwb_write_close(zilog_t *zilog, lwb_t *lwb,
                                  lwb_state_t state) {
  ...
  if (zilog->zl_cur_left > 0) {
    ...
    if (zilog->zl_parallel) {
      plan2 =
          zil_lwb_plan(zilog, zilog->zl_cur_left + zil_lwb_predict(zilog), &m);
      if (plan < plan2)
        plan = plan2;
    }
  } else {
  ...
  DTRACE_PROBE3(zil__block__size, zilog_t *, zilog, uint64_t, blksz, uint64_t,
                plan);
...
static void zil_lwb_write_issue(zilog_t *zilog, lwb_t *lwb) {
  ...
  lwb->lwb_root_zio =
      zio_root(spa, zil_lwb_flush_vdevs_done, lwb, ZIO_FLAG_CANFAIL);
...
next_lwb:
  ...
  if (lwb->lwb_error == 0) {
    ...
    SET_BOOKMARK(&zb, lwb->lwb_blk.blk_cksum.zc_word[ZIL_ZC_OBJSET],
                 ...
                 lwb->lwb_blk.blk_cksum.zc_word[ZIL_ZC_SEQ]);
    lwb->lwb_write_zio = zio_rewrite(lwb->lwb_root_zio, spa, 0, &lwb->lwb_blk,
                                     lwb_abd, lwb->lwb_sz, zil_lwb_write_done,
  ...
  error = lwb->lwb_error;
  if (error == 0) {
    error = zio_alloc_zil(spa, zilog->zl_os, txg, bp, nlwb->lwb_sz, &slog);
  }
  if (error == 0) {
    ASSERT3U(BP_GET_LOGICAL_BIRTH(bp), ==, txg);
    BP_SET_CHECKSUM(bp,
                    nlwb->lwb_slim ? ZIO_CHECKSUM_ZILOG2 : ZIO_CHECKSUM_ZILOG);
  ...
  if (lwb->lwb_slog) {
    ...
    ZIL_STAT_INCR(zilog, zil_itx_metaslab_slog_bytes, lwb->lwb_nused);
    ZIL_STAT_INCR(zilog, zil_itx_metaslab_slog_write, wsz);
    ZIL_STAT_INCR(zilog, zil_itx_metaslab_slog_alloc,
                  BP_GET_LSIZE(&lwb->lwb_blk));
  } else {
    ZIL_STAT_BUMP(zilog, zil_itx_metaslab_normal_count);
    ZIL_STAT_INCR(zilog, zil_itx_metaslab_normal_bytes, lwb->lwb_nused);
    ZIL_STAT_INCR(zilog, zil_itx_metaslab_normal_write, wsz);
 ...
 */
uint64_t zil_max_log_data(zilog_t *zilog, size_t hdrsize) {
...
static uint64_t zil_itx_record_size(itx_t *itx) {
  lr_t *lr = &itx->itx_lr;
...
static uint64_t zil_itx_data_size(itx_t *itx) {
  lr_t *lr = &itx->itx_lr;
  lr_write_t *lrw = (lr_write_t *)lr;

  if (lr->lrc_txtype == TX_WRITE && itx->itx_wr_state == WR_NEED_COPY) {
    ASSERT3U(lr->lrc_reclen, ==, sizeof(lr_write_t));
    return (P2ROUNDUP_TYPED(lrw->lr_length, sizeof(uint64_t), uint64_t));
  }
  return (0);
}

static uint64_t zil_itx_full_size(itx_t *itx) {
  lr_t *lr = &itx->itx_lr;
 ...
 * to chain more lwbs.
 */
static lwb_t *zil_lwb_assign(zilog_t *zilog, lwb_t *lwb, itx_t *itx,
                             list_t *ilwbs) {
  itx_t *citx;
  lr_t *lr, *clr;
  lr_write_t *lrw;
  ...
  zfs_dbgmsg(" lwb->lwb_issued_txg=%llu, lwb->lwb_alloc_txg=%llu, "
             "lwb->lwb_max_txg=%llu\n",
             (u_longlong_t)lwb->lwb_issued_txg,
             (u_longlong_t)lwb->lwb_alloc_txg, (u_longlong_t)lwb->lwb_max_txg);
...
cont:
  ...
  if (reclen > lwb_sp ||
      (reclen + dlen > lwb_sp && lwb_sp < zil_max_waste_space(zilog) &&
  ...
  if (lr->lrc_txtype == TX_WRITE && lr->lrc_txg > spa_freeze_txg(zilog->zl_spa))
...
static void zil_lwb_commit(zilog_t *zilog, lwb_t *lwb, itx_t *itx) {
  ...
  if (lr->lrc_txtype == TX_WRITE) {
    if (itx->itx_wr_state == WR_COPIED) {
      ...
      ZIL_STAT_INCR(zilog, zil_itx_copied_bytes, lrw->lr_length);
    } else {
      char *dbuf;
      int error;
      ...
      if (itx->itx_wr_state == WR_NEED_COPY) {
        ...
        ZIL_STAT_INCR(zilog, zil_itx_needcopy_bytes, dlen);
      } else {
        ASSERT3S(itx->itx_wr_state, ==, WR_INDIRECT);
        dbuf = NULL;
        ZIL_STAT_BUMP(zilog, zil_itx_indirect_count);
        ZIL_STAT_INCR(zilog, zil_itx_indirect_bytes, lrw->lr_length);
        if (lwb->lwb_child_zio == NULL) {
          lwb->lwb_child_zio =
              zio_null(NULL, zilog->zl_spa, NULL, NULL, NULL, ZIO_FLAG_CANFAIL);
        }
      }
       ...
       */
      error = zilog->zl_get_data(itx->itx_private, itx->itx_gen, lrwb, dbuf,
                                 lwb, lwb->lwb_child_zio);
      if (dbuf != NULL && error == 0) {
        /* Zero any padding bytes in the last block. */
        memset((char *)dbuf + lrwb->lr_length, 0, dlen - lrwb->lr_length);
      }
  ...
  zfs_dbgmsg(
      " lwb=%p, fill the actual transaction data into the lwb for lrc_txg=%llu "
      "and lrc_seq=%llu (following zil_lwb_assign) lwb->lwb_issued_txg=%llu, "
      "lwb->lwb_alloc_txg=%llu, lwb->lwb_max_txg=%llu\n",
      (void *)lwb, (u_longlong_t)lr->lrc_txg, (u_longlong_t)lr->lrc_seq,
      (u_longlong_t)lwb->lwb_issued_txg, (u_longlong_t)lwb->lwb_alloc_txg,
      (u_longlong_t)lwb->lwb_max_txg);
#if 0
	if (lr->lrc_txtype == TX_WRITE) {
...
void zil_itx_destroy(itx_t *itx) {
  ...
  ASSERT3U(itx->itx_lr.lrc_reclen, ==, itx->itx_size - offsetof(itx_t, itx_lr));
...
void zil_itx_assign(zilog_t *zilog, itx_t *itx, dmu_tx_t *tx) {
  ...
  if (itxg->itxg_txg != txg) {
    ...
    avl_create(&itxs->i_async_tree, zil_aitx_compare, sizeof(itx_async_node_t),
 ...
 */
static uint64_t zil_get_commit_list(zilog_t *zilog) {
...
static void zil_burst_done(zilog_t *zilog) {
  if (!list_is_empty(&zilog->zl_itx_commit_list) || zilog->zl_cur_size == 0)
    return;
  ...
  zilog->zl_prev_opt[r] =
      zil_lwb_plan(zilog, zilog->zl_cur_size, &zilog->zl_prev_min[r]);
 ...
 */
static void zil_process_commit_list(zilog_t *zilog, zil_commit_waiter_t *zcw,
                                    list_t *ilwbs) {
  spa_t *spa = zilog->zl_spa;
  list_t nolwb_itxs;
  ...
  if (lwb == NULL) {
    ...
    } else if ((plwb = list_prev(&zilog->zl_lwb_list, lwb)) != NULL &&
               plwb->lwb_state != LWB_STATE_FLUSH_DONE) {
      zilog->zl_parallel = MAX(zilog->zl_parallel, ZIL_BURSTS / 2);
    }
  }
  ...
  while ((itx = list_remove_head(&zilog->zl_itx_commit_list)) != NULL) {
    ...
    if (frozen || !synced || lrc->lrc_txtype == TX_COMMIT) {
      ...
      if (lwb != NULL) {
        ...
        } else if ((zcw->zcw_lwb != NULL && zcw->zcw_lwb != lwb) ||
                   zcw->zcw_done) {
          /*
 ...
 */
static uint64_t zil_commit_writer(zilog_t *zilog, zil_commit_waiter_t *zcw) {
  list_t ilwbs;
  lwb_t *lwb;
  uint64_t wtxg = 0;
...
static void zil_commit_waiter_timeout(zilog_t *zilog,
                                      zil_commit_waiter_t *zcw) {
  ASSERT(!MUTEX_HELD(&zilog->zl_issuer_lock));
...
static void zil_commit_waiter(zilog_t *zilog, zil_commit_waiter_t *zcw) {
  ...
  while (!zcw->zcw_done) {
    ...
    if (lwb != NULL && lwb->lwb_state == LWB_STATE_OPENED) {
       ...
       */
      int rc = cv_timedwait_hires(&zcw->zcw_cv, &zcw->zcw_lock, wakeup,
...
static zil_commit_waiter_t *zil_alloc_commit_waiter(void) {
  ...
  // ccf_waiter_t initialization
  zcw->zcw_ccf_waiter_ptr = kmem_alloc(sizeof(ccf_waiter_t), KM_SLEEP);
  zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr =
      kmem_alloc(sizeof(ccf_cond_var_t), KM_SLEEP);
  cv_init(&(zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_ccf_cv), NULL, CV_DEFAULT,
          NULL);
  mutex_init(&(zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_ccf_lock), NULL,
             MUTEX_DEFAULT, NULL);
...
static void zil_free_commit_waiter(zil_commit_waiter_t *zcw) {
  /*
          zfs_dbgmsg(" [Before blocking] zcw->block_id  %llu is safe to ccf =
     %s",\
                  (u_longlong_t)zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id,
     \ (zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_ccf_acked == B_TRUE) ?
     "true" : "false");
  */
  mutex_enter(&zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_ccf_lock);
  if (zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id != -1) {
    /*
                    zfs_dbgmsg(" [Before waiting] zcw->block_id  %llu is safe to
       ccf = %s",\
                            (u_longlong_t)zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id,
       \ (zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_ccf_acked == B_TRUE) ?
       "true" : "false");
    */
    while (zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_ccf_acked ==
           B_FALSE) {
      cv_wait(&zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_ccf_cv,
              &zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_ccf_lock);
    }
  }
  mutex_exit(&zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_ccf_lock);

#if 0
	zfs_dbgmsg(" [After waking up] zcw->block_id is safe to ccf: %llu is safe to ccf = %s",\
	    (u_longlong_t)zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id, \
		(zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_ccf_acked == B_TRUE) ? "true" : "false");
#endif
  ASSERT(!list_link_active(&zcw->zcw_node));
  ...
  if (zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id != -1) {
    ...
    if (start_time == 0) {
      zfs_dbgmsg(
          " [ERROR@Finalization] valid zcw->block_id=%d is safe to ccf = %s "
          "latency(us)=%llu",
          (int)zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id,
          (zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_ccf_acked == B_TRUE)
              ? "true"
              : "false",
          (u_longlong_t)latency);
    } else {
      zfs_dbgmsg(
          " [Finalization] valid zcw->block_id=%d is safe to ccf = %s "
          "latency=%llu us",
          (int)zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id,
          (zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_ccf_acked == B_TRUE)
              ? "true"
    ...
    ASSERT3B(zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_ccf_acked, ==,
             B_TRUE);
  } else {
    zfs_dbgmsg(
        " [Finalization] empty zil zcw->block_id %d is safe to ccf = %s",
        (int)zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_id,
        (zcw->zcw_ccf_waiter_ptr->zcw_ccf_ptr->zcw_block_ccf_acked == B_TRUE)
            ? "true"
            : "false");
...
void zil_commit_impl(zilog_t *zilog, uint64_t foid) {
  ...
  if (zcw->zcw_zio_error != 0) {
    ...
    DTRACE_PROBE2(zil__commit__io__error, zilog_t *, zilog,
                  zil_commit_waiter_t *, zcw);
    txg_wait_synced(zilog->zl_dmu_pool, 0);
  } else if (wtxg != 0) {
    ZIL_STAT_BUMP(zilog, zil_commit_suspend_count);
...
void zil_sync(zilog_t *zilog, dmu_tx_t *tx) {
  ...
  zfs_dbgmsg(" *************** start (txg=%llu, BP %s hole)***************\n",
             (u_longlong_t)txg,
             (BP_IS_HOLE(&(zh->zh_log))) ? "is a" : "is not a");
  ...
  if (zilog->zl_destroy_txg == txg) {
    ...
    memset(zilog->zl_replayed_seq, 0, sizeof(zilog->zl_replayed_seq));
    ...
    if (zilog->zl_keep_first) {
       ...
       */
      if (dsl_dataset_feature_is_active(ds, SPA_FEATURE_ZILSAXATTR))
        dsl_dataset_deactivate_feature(ds, SPA_FEATURE_ZILSAXATTR, tx);
    }
  }
  ...
  while ((lwb = list_head(&zilog->zl_lwb_list)) != NULL) {
    ...
    zfs_dbgmsg(
        " zilog=%p lwb_blk->cksum_seq_no=%016llx:%016llx:%016llx:%016llx (%llu)\
    ...
    if (lwb->lwb_state != LWB_STATE_FLUSH_DONE || lwb->lwb_alloc_txg > txg ||
        lwb->lwb_max_txg > txg)
      break;

    zfs_dbgmsg(
        " TXG_DEFER_SIZE=%llu\t remove lwb_blk->cksum_seq_no=%016llx:%016llx:%016llx:%016llx \
			in txg=%llu w/ lwb->lwb_alloc_txg=%llu lwb->lwb_max_txg=%llu\nDVA=<%llx:%llx:%llx>\n",
        (u_longlong_t)TXG_DEFER_SIZE,
        (u_longlong_t)lwb->lwb_blk.blk_cksum.zc_word[0],
        (u_longlong_t)lwb->lwb_blk.blk_cksum.zc_word[1],
        (u_longlong_t)lwb->lwb_blk.blk_cksum.zc_word[2],
        (u_longlong_t)lwb->lwb_blk.blk_cksum.zc_word[ZIL_ZC_SEQ],
        (u_longlong_t)txg, (u_longlong_t)lwb->lwb_alloc_txg,
        (u_longlong_t)lwb->lwb_max_txg,
        (u_longlong_t)DVA_GET_VDEV(lwb->lwb_blk.blk_dva),
        (u_longlong_t)DVA_GET_OFFSET(lwb->lwb_blk.blk_dva),
        (u_longlong_t)DVA_GET_ASIZE(lwb->lwb_blk.blk_dva));
  ...
  zfs_dbgmsg(
      " *************** end (txg=%llu\tBP_IS_HOLE=%d\tblk_seqno=%016llx:%016llx:%016llx:%016llx\t \
		DVA=<%llu:%llx:%llx>) ***************\n",
      (u_longlong_t)txg, BP_IS_HOLE(&(zh->zh_log)),
      (u_longlong_t)zh->zh_log.blk_cksum.zc_word[0],
      (u_longlong_t)zh->zh_log.blk_cksum.zc_word[1],
      (u_longlong_t)zh->zh_log.blk_cksum.zc_word[2],
...
static void print_serialized_zil_header(const zil_header_t *zh) {
  ...
  */
  ...
  zfs_dbgmsg("ZIL header\t claim_txg %llu\t "
             "claim_blk_seq %llu\t claim_lr_seq %llu\treplay_seq %llu\t flags "
             "0x%llx\n",
             (u_longlong_t)zh->zh_claim_txg, (u_longlong_t)zh->zh_claim_blk_seq,
             (u_longlong_t)zh->zh_claim_lr_seq, (u_longlong_t)zh->zh_replay_seq,
             (u_longlong_t)zh->zh_flags);
  if (BP_IS_HOLE(&zh->zh_log)) {
...
static int zil_lwb_cons(void *vbuf, void *unused, int kmflag) {
  ...
  list_create(&lwb->lwb_waiters, sizeof(zil_commit_waiter_t),
              offsetof(zil_commit_waiter_t, zcw_node));
  avl_create(&lwb->lwb_vdev_tree, zil_lwb_vdev_compare, sizeof(zil_vdev_node_t),
             offsetof(zil_vdev_node_t, zv_node));
...
void zil_init(void) {
  zil_lwb_cache =
      kmem_cache_create("zil_lwb_cache", sizeof(lwb_t), 0, zil_lwb_cons,
                        zil_lwb_dest, NULL, NULL, NULL, 0);

  zil_zcw_cache =
      kmem_cache_create("zil_zcw_cache", sizeof(zil_commit_waiter_t), 0, NULL,
                        NULL, NULL, NULL, NULL, 0);

  zil_sums_init(&zil_sums_global);
  zil_kstats_global = kstat_create("zfs", 0, "zil", "misc", KSTAT_TYPE_NAMED,
                                   sizeof(zil_stats) / sizeof(kstat_named_t),
                                   KSTAT_FLAG_VIRTUAL);
...
void zil_set_sync(zilog_t *zilog, uint64_t sync) { zilog->zl_sync = sync; }
...
zilog_t *zil_alloc(objset_t *os, zil_header_t *zh_phys) {
  ...
  zilog->zl_max_block_size =
      MIN(MAX(P2ALIGN_TYPED(zil_maxblocksize, ZIL_MIN_BLKSZ, uint64_t),
              ZIL_MIN_BLKSZ),
  ...
  for (int i = 0; i < ZIL_BURSTS; i++) {
    zilog->zl_prev_opt[i] = zilog->zl_max_block_size - sizeof(zil_chain_t);
  }
...
static int zil_replay_error(zilog_t *zilog, const lr_t *lr, int error) {
  ...
  cmn_err(CE_WARN,
          ...
          error, name, (u_longlong_t)lr->lrc_seq,
...
static int zil_replay_log_record(zilog_t *zilog, const lr_t *lr, void *zra,
                                 uint64_t claim_txg) {
 ...
 * Return B_TRUE if there were any entries to replay.
 */
boolean_t zil_replay(objset_t *os, void *arg,
