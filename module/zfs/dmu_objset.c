    ...
    objset_t **osp)
{
	...
	zfs_dbgmsg(" name=%s DVA=<%llu:%llx:%llx>\n", name, \
	(u_longlong_t)DVA_GET_VDEV(bp->blk_dva),  (u_longlong_t)DVA_GET_OFFSET(bp->blk_dva), \
	(u_longlong_t)DVA_GET_ASIZE(bp->blk_dva));
	...
	if (ds != NULL) {
		...
		if (!ds->ds_is_snapshot) {
			...
			if (err == 0) {
				zfs_dbgmsg(" we might have initialized os_zil\n");
				err = dsl_prop_register(ds,
	...
	zfs_dbgmsg(" name=%s\tos->os_zil_header.zh_claim_txg=%llu os->os_zil_header.zh_claim_blk_seq=%llu, os->os_zil_header.zh_claim_lr_seq=%llu, BP_IS_HOLE=%d\n", name, (u_longlong_t)(os->os_zil_header.zh_claim_txg), (u_longlong_t)(os->os_zil_header.zh_claim_blk_seq), (u_longlong_t)(os->os_zil_header.zh_claim_lr_seq), BP_IS_HOLE(&(os->os_zil_header.zh_log)));
	zfs_dbgmsg(" name=%s zh_log DVA=<%llu:%llx:%llx>\n", name, \
		(u_longlong_t)DVA_GET_VDEV(os->os_zil_header.zh_log.blk_dva),  (u_longlong_t)DVA_GET_OFFSET(os->os_zil_header.zh_log.blk_dva), \
		(u_longlong_t)DVA_GET_ASIZE(os->os_zil_header.zh_log.blk_dva));
...
dmu_objset_from_ds(dsl_dataset_t *ds, objset_t **osp)
{
	...
	zfs_dbgmsg(" This is where we read the dataset .. (an parse the ZIL, etc.)\n");
...
dmu_objset_sync(objset_t *os, zio_t *pio, dmu_tx_t *tx)
{
	...
	*blkptr_copy = *os->os_rootbp;
	...
	zfs_dbgmsg(" name=%s txg=%llu\n",  name, (u_longlong_t)tx->tx_txg);
	...
	*/
	zfs_dbgmsg(" ZIL blocks up to this tx=%llu are freed (os->os_zil_header.zh_claim_txg=%llu, os->os_zil_header.zh_claim_blk_seq=%llu, os->os_zil_header.zh_claim_lr_seq=%llu, BP_IS_HOLE=%d)\n", (u_longlong_t)dmu_tx_get_txg(tx), (u_longlong_t)(os->os_zil_header.zh_claim_txg), (u_longlong_t)(os->os_zil_header.zh_claim_blk_seq), (u_longlong_t)(os->os_zil_header.zh_claim_lr_seq), BP_IS_HOLE(&(os->os_zil_header.zh_log)));
	zio_nowait(zio);
}
