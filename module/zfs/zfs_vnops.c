...
zfs_read(struct znode *zp, zfs_uio_t *uio, int ioflag, cred_t *cr)
{
	...
	if (zfsvfs->z_log &&
		...
		zfs_dbgmsg(" we commit before reading (ZFS_SYNC_ALWAYS)\n");
		zil_commit(zfsvfs->z_log, zp->z_id);
	}
	...
	while (n > 0) {
		...
		if (zn_has_cached_data(zp, zfs_uio_offset(uio),
			...
			zfs_dbgmsg(" zn_has_cached_data() [mappedread()]\n");
			error = mappedread(zp, nbytes, uio);
		} else {
			error = dmu_read_uio_dbuf(sa_get_db(zp->z_sa_hdl),
			    uio, nbytes);
			zfs_dbgmsg(" we read through dmu_read_uio_dbuf()\n");
...
{
	zfs_dbgmsg(" please check this w/ zil_lwb_commit\n");
	zfsvfs_t *zfsvfs = arg;
