...
retry:
	...
	if (ub->ub_txg < txg) {
		...
		zfs_dbgmsg(" uberblock_update changed=%d for txg=%llu\n", changed, (u_longlong_t)txg);

		if (!changed && list_is_empty(&spa->spa_config_dirty_list) &&
		    !spa_multihost(spa)) {
			zfs_dbgmsg(" there is nothing to do txg=%llu\n", (u_longlong_t)txg);
	...
	if (txg > spa_freeze_txg(spa)) {
		ccf_commit_cmts(ccf_zil_header_commitments, ZIL_HEAD_COMMITMENT);
		zfs_dbgmsg(" pool is frozen in spa_freeze_txg(spa)=%llu (now at txg=%llu)\n", (u_longlong_t) spa_freeze_txg(spa), (u_longlong_t) txg);
		return (0);
	}
