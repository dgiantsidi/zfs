...
static void vdev_label_read(zio_t *zio, vdev_t *vd, int l, abd_t *buf,
                            uint64_t offset, uint64_t size,
                            zio_done_func_t *done, void *private, int flags) {
  ASSERT(spa_config_held(zio->io_spa, SCL_STATE, RW_READER) == SCL_STATE ||
         spa_config_held(zio->io_spa, SCL_STATE, RW_WRITER) == SCL_STATE);
  ASSERT(flags & ZIO_FLAG_CONFIG_WRITER);

  zio_nowait(zio_read_phys(zio, vd,
                           vdev_label_offset(vd->vdev_psize, l, offset), size,
                           buf, ZIO_CHECKSUM_LABEL, done, private,
                           ZIO_PRIORITY_SYNC_READ, flags, B_TRUE));
}

void vdev_label_write(zio_t *zio, vdev_t *vd, int l, abd_t *buf,
 ...
 */
void vdev_config_generate_stats(vdev_t *vd, nvlist_t *nv) {
  nvlist_t *nvx;
  vdev_stat_t *vs;
  vdev_stat_ex_t *vsx;
  ...
  fnvlist_add_uint64_array(nv, ZPOOL_CONFIG_VDEV_STATS, (uint64_t *)vs,
  ...
  fnvlist_add_uint64_array(
      nvx, ZPOOL_CONFIG_VDEV_AGG_REBUILD_HISTO,
...
static void root_vdev_actions_getprogress(vdev_t *vd, nvlist_t *nvl) {
  ...
  if (spa_scan_get_stats(spa, &ps) == 0) {
    fnvlist_add_uint64_array(nvl, ZPOOL_CONFIG_SCAN_STATS, (uint64_t *)&ps,
                             sizeof(pool_scan_stat_t) / sizeof(uint64_t));
  }

  pool_removal_stat_t prs;
  if (spa_removal_get_stats(spa, &prs) == 0) {
    fnvlist_add_uint64_array(nvl, ZPOOL_CONFIG_REMOVAL_STATS, (uint64_t *)&prs,
                             sizeof(prs) / sizeof(uint64_t));
  }

  pool_checkpoint_stat_t pcs;
  if (spa_checkpoint_get_stats(spa, &pcs) == 0) {
    fnvlist_add_uint64_array(nvl, ZPOOL_CONFIG_CHECKPOINT_STATS,
                             (uint64_t *)&pcs, sizeof(pcs) / sizeof(uint64_t));
  }

  pool_raidz_expand_stat_t pres;
  if (spa_raidz_expand_get_stats(spa, &pres) == 0) {
    fnvlist_add_uint64_array(nvl, ZPOOL_CONFIG_RAIDZ_EXPAND_STATS,
                             (uint64_t *)&pres,
                             sizeof(pres) / sizeof(uint64_t));
  }
}

static void top_vdev_actions_getprogress(vdev_t *vd, nvlist_t *nvl) {
  if (vd == vd->vdev_top) {
    vdev_rebuild_stat_t vrs;
    if (vdev_rebuild_get_stats(vd, &vrs) == 0) {
      fnvlist_add_uint64_array(nvl, ZPOOL_CONFIG_REBUILD_STATS,
                               (uint64_t *)&vrs,
 ...
 */
nvlist_t *vdev_config_generate(spa_t *spa, vdev_t *vd, boolean_t getstats,
  ...
  if (vd->vdev_wholedisk != -1ULL) {
    fnvlist_add_uint64(nv, ZPOOL_CONFIG_WHOLE_DISK, vd->vdev_wholedisk);
  }
  ...
  if (!(flags & (VDEV_CONFIG_SPARE | VDEV_CONFIG_L2CACHE)) &&
    ...
    if (vd->vdev_noalloc) {
      fnvlist_add_uint64(nv, ZPOOL_CONFIG_NONALLOCATING, vd->vdev_noalloc);
    }
    ...
    if (vd->vdev_removing && !vd->vdev_islog) {
      fnvlist_add_uint64(nv, ZPOOL_CONFIG_REMOVING, vd->vdev_removing);
    }
    ...
    if (getstats && vd->vdev_alloc_bias != VDEV_BIAS_NONE) {
      ...
      fnvlist_add_string(nv, ZPOOL_CONFIG_ALLOCATION_BIAS, bias);
    }
  }

  if (vd->vdev_dtl_sm != NULL) {
    fnvlist_add_uint64(nv, ZPOOL_CONFIG_DTL, space_map_object(vd->vdev_dtl_sm));
  }
  ...
  if (flags & VDEV_CONFIG_MOS) {
    if (vd->vdev_leaf_zap != 0) {
      ...
      fnvlist_add_uint64(nv, ZPOOL_CONFIG_VDEV_LEAF_ZAP, vd->vdev_leaf_zap);
    }

    if (vd->vdev_top_zap != 0) {
      ASSERT(vd == vd->vdev_top);
      fnvlist_add_uint64(nv, ZPOOL_CONFIG_VDEV_TOP_ZAP, vd->vdev_top_zap);
    }

    if (vd->vdev_ops == &vdev_root_ops && vd->vdev_root_zap != 0 &&
        spa_feature_is_active(vd->vdev_spa, SPA_FEATURE_AVZ_V2)) {
      fnvlist_add_uint64(nv, ZPOOL_CONFIG_VDEV_ROOT_ZAP, vd->vdev_root_zap);
    }
  ...
  if (getstats) {
    ...
    if (vd->vdev_mg != NULL &&
      ...
      for (int i = 0; i < ZFS_RANGE_TREE_HISTOGRAM_SIZE; i++) {
        if (i + 1 < highbit64(vdev_removal_max_span) - 1) {
          to_alloc += vd->vdev_mg->mg_histogram[i] << (i + 1);
        } else {
  ...
  if (!vd->vdev_ops->vdev_op_leaf) {
    ...
    child = kmem_alloc(vd->vdev_children * sizeof(nvlist_t *), KM_SLEEP);

    for (c = 0; c < vd->vdev_children; c++) {
      child[c] = vdev_config_generate(spa, vd->vdev_child[c], getstats, flags);
    }
    ...
    if (vd->vdev_resilver_txg != 0)
      fnvlist_add_uint64(nv, ZPOOL_CONFIG_RESILVER_TXG, vd->vdev_resilver_txg);
    if (vd->vdev_rebuild_txg != 0)
      fnvlist_add_uint64(nv, ZPOOL_CONFIG_REBUILD_TXG, vd->vdev_rebuild_txg);
    if (vd->vdev_faulted)
    ...
    if (vd->vdev_splitting && vd->vdev_orig_guid != 0LL) {
      fnvlist_add_uint64(nv, ZPOOL_CONFIG_ORIG_GUID, vd->vdev_orig_guid);
    }
  }
...
void vdev_top_config_generate(spa_t *spa, nvlist_t *config) {
  ...
  if (idx) {
    VERIFY(nvlist_add_uint64_array(config, ZPOOL_CONFIG_HOLE_ARRAY, array,
                                   idx) == 0);
  }
...
nvlist_t *vdev_label_read_config(vdev_t *vd, uint64_t txg) {
  ...
  int error = 0;
  int flags = ZIO_FLAG_CONFIG_WRITER | ZIO_FLAG_CANFAIL | ZIO_FLAG_SPECULATIVE;
...
retry:
  ...
  for (int l = 0; l < VDEV_LABELS; l++) {
    ...
    if (zio_wait(zio[l]) == 0 &&
        nvlist_unpack(vp[l]->vp_nvlist, sizeof(vp[l]->vp_nvlist), &label, 0) ==
            0) {
      /*
       ...
       * encounter.
       */
      error = nvlist_lookup_uint64(label, ZPOOL_CONFIG_POOL_TXG, &label_txg);
  ...
  if (config == NULL && label_txg != 0) {
    vdev_dbgmsg(vd,
                ...
                "(%llu > %llu)",
                (u_longlong_t)label_txg, (u_longlong_t)txg);
  }
 ...
 */
static boolean_t vdev_inuse(vdev_t *vd, uint64_t crtxg, vdev_labeltype_t reason,
  ...
  (void)nvlist_lookup_uint64(label, ZPOOL_CONFIG_CREATE_TXG, &vdtxg);
  ...
  if (state != POOL_STATE_SPARE && state != POOL_STATE_L2CACHE &&
      (nvlist_lookup_uint64(label, ZPOOL_CONFIG_POOL_GUID, &pool_guid) != 0 ||
       nvlist_lookup_uint64(label, ZPOOL_CONFIG_POOL_TXG, &txg) != 0)) {
    nvlist_free(label);
    return (B_FALSE);
  }
   ...
   * transaction.
   */
  if (state != POOL_STATE_SPARE && state != POOL_STATE_L2CACHE && txg == 0 &&
      vdtxg == crtxg)
    return (B_TRUE);
  ...
  if (spa_spare_exists(device_guid, &spare_pool, NULL) ||
    ...
    case VDEV_LABEL_REPLACE:
      return (!spa_has_spare(spa, device_guid) || spare_pool != 0ULL);
...
static nvlist_t *vdev_aux_label_generate(vdev_t *vd, boolean_t reason_spare) {
  /*
  ...
  fnvlist_add_uint64(label, ZPOOL_CONFIG_VERSION, spa_version(vd->vdev_spa));
  fnvlist_add_uint64(label, ZPOOL_CONFIG_POOL_STATE,
                     reason_spare ? POOL_STATE_SPARE : POOL_STATE_L2CACHE);
  ...
  if (vd->vdev_physpath != NULL) {
    fnvlist_add_string(label, ZPOOL_CONFIG_PHYS_PATH, vd->vdev_physpath);
  }
  return (label);
}
...
int vdev_label_init(vdev_t *vd, uint64_t crtxg, vdev_labeltype_t reason) {
  ...
  boolean_t reason_spare = (reason == VDEV_LABEL_SPARE ||
                            (reason == VDEV_LABEL_REMOVE && vd->vdev_isspare));
  boolean_t reason_l2cache =
  ...
  for (int c = 0; c < vd->vdev_children; c++)
    if ((error = vdev_label_init(vd->vdev_child[c], crtxg, reason)) != 0)
      return (error);
  ...
  if (reason != VDEV_LABEL_REMOVE && reason != VDEV_LABEL_L2CACHE &&
    ...
    if (reason == VDEV_LABEL_SPARE)
      return (0);
    ASSERT(reason == VDEV_LABEL_REPLACE || reason == VDEV_LABEL_SPLIT);
  }
...
retry:
  ...
  for (int l = 0; l < VDEV_LABELS; l++) {
    ...
    vdev_label_write(zio, vd, l, vp_abd, offsetof(vdev_label_t, vl_vdev_phys),
     ...
     */
    vdev_label_write(zio, vd, l, bootenv, offsetof(vdev_label_t, vl_be),
                     VDEV_PAD_SIZE, NULL, NULL, flags);

    vdev_label_write(zio, vd, l, ub_abd, offsetof(vdev_label_t, vl_uberblock),
  ...
  if (error == 0 && !vd->vdev_isl2cache &&
      (reason == VDEV_LABEL_L2CACHE || spa_l2cache_exists(vd->vdev_guid, NULL)))
    spa_l2cache_add(vd);
 ...
 */
static void vdev_label_read_bootenv_done(zio_t *zio) {
...
static void vdev_label_read_bootenv_impl(zio_t *zio, vdev_t *vd, int flags) {
  for (int c = 0; c < vd->vdev_children; c++)
  ...
  if (vd->vdev_ops->vdev_op_leaf && vdev_readable(vd)) {
    for (int l = 0; l < VDEV_LABELS; l++) {
      vdev_label_read(zio, vd, l, abd_alloc_linear(VDEV_PAD_SIZE, B_FALSE),
...
int vdev_label_read_bootenv(vdev_t *rvd, nvlist_t *bootenv) {
  nvlist_t *config;
  ...
  if (abd != NULL) {
    ...
    case VB_RAW:
      ...
      fnvlist_add_string(bootenv, GRUB_ENVMAP, vbe->vbe_bootenv);
      break;

    case VB_NVLIST:
      err =
          nvlist_unpack(vbe->vbe_bootenv, sizeof(vbe->vbe_bootenv), &config, 0);
      if (err == 0) {
    ...
    default:
      ...
      buf = abd_to_buf(abd);
      if (*buf == '\0') {
        fnvlist_add_uint64(bootenv, BOOTENV_VERSION, VB_NVLIST);
        break;
      }
...
int vdev_label_write_bootenv(vdev_t *vd, nvlist_t *env) {
  zio_t *zio;
  spa_t *spa = vd->vdev_spa;
  ...
  if (!vd->vdev_ops->vdev_op_leaf || vdev_is_dead(vd) || !vdev_writeable(vd)) {
    return (error);
  }
...
retry:
  zio = zio_root(spa, NULL, NULL, flags);
  for (int l = 0; l < VDEV_LABELS; l++) {
    vdev_label_write(zio, vd, l, abd, offsetof(vdev_label_t, vl_be),
                     VDEV_PAD_SIZE, NULL, NULL, flags);
  }
 ...
 */
static int vdev_uberblock_compare(const uberblock_t *ub1,
                                  const uberblock_t *ub2) {
    ...
    for (int l = 0; l < VDEV_LABELS; l++) {
      for (int n = 0; n < VDEV_UBERBLOCK_COUNT(vd); n++) {
        vdev_label_read(zio, vd, l,
                        abd_alloc_linear(VDEV_UBERBLOCK_SIZE(vd), B_TRUE),
  ...
  *config = NULL;
  ...
  if (cb.ubl_vd != NULL) {
    vdev_dbgmsg(cb.ubl_vd,
                ...
                "txg %llu",
                spa->spa_name, (u_longlong_t)ub->ub_txg);

    if (ub->ub_raidz_reflow_info != cb.ubl_latest.ub_raidz_reflow_info) {
      vdev_dbgmsg(cb.ubl_vd,
...
static void vdev_copy_uberblocks(vdev_t *vd) {
  abd_t *ub_abd;
  zio_t *write_zio;
  int locks = (SCL_L2ARC | SCL_ZIO);
  int flags = ZIO_FLAG_CONFIG_WRITER | ZIO_FLAG_CANFAIL | ZIO_FLAG_SPECULATIVE;

  ASSERT(spa_config_held(vd->vdev_spa, SCL_STATE, RW_READER) == SCL_STATE);
  ASSERT(vd->vdev_ops->vdev_op_leaf);
  ...
  for (int n = 0; n < VDEV_UBERBLOCK_COUNT(vd); n++) {
    ...
    vdev_label_read(zio, vd, src_label, ub_abd, VDEV_UBERBLOCK_OFFSET(vd, n),
    ...
    for (int l = 2; l < VDEV_LABELS; l++)
      vdev_label_write(write_zio, vd, l, ub_abd, VDEV_UBERBLOCK_OFFSET(vd, n),
 ...
 */
static void vdev_uberblock_sync(zio_t *zio, uint64_t *good_writes,
                                uberblock_t *ub, vdev_t *vd, int flags) {
  for (uint64_t c = 0; c < vd->vdev_children; c++) {
    vdev_uberblock_sync(zio, good_writes, ub, vd->vdev_child[c], flags);
  }
  ...
  for (int l = 0; l < VDEV_LABELS; l++)
    vdev_label_write(zio, vd, l, ub_abd, VDEV_UBERBLOCK_OFFSET(vd, n),
...
int vdev_uberblock_sync_list(vdev_t **svd, int svdcount, uberblock_t *ub,
                             int flags) {
  ...
  if (spa->spa_aux_sync_uber) {
    for (int v = 0; v < spa->spa_spares.sav_count; v++) {
      vdev_uberblock_sync(zio, &good_writes, ub, spa->spa_spares.sav_vdevs[v],
                          flags);
    }
    for (int v = 0; v < spa->spa_l2cache.sav_count; v++) {
      vdev_uberblock_sync(zio, &good_writes, ub, spa->spa_l2cache.sav_vdevs[v],
                          flags);
    }
  }
  (void)zio_wait(zio);
 ...
 */
static void vdev_label_sync(zio_t *zio, uint64_t *good_writes, vdev_t *vd,
                            int l, uint64_t txg, int flags) {
  nvlist_t *label;
  vdev_phys_t *vp;
  abd_t *vp_abd;
  char *buf;
  ...
  for (int c = 0; c < vd->vdev_children; c++) {
    vdev_label_sync(zio, good_writes, vd->vdev_child[c], l, txg, flags);
  }
  ...
  if (!nvlist_pack(label, &buf, &buflen, NV_ENCODE_XDR, KM_SLEEP)) {
    for (; l < VDEV_LABELS; l += 2) {
      vdev_label_write(zio, vd, l, vp_abd, offsetof(vdev_label_t, vl_vdev_phys),
                       sizeof(vdev_phys_t), vdev_label_sync_done, good_writes,
                       flags | ZIO_FLAG_DONT_PROPAGATE);
    }
  }
...
static int vdev_label_sync_list(spa_t *spa, int l, uint64_t txg, int flags) {
  ...
  for (int i = 0; i < 2; i++) {
    for (int v = 0; v < sav[i]->sav_count; v++) {
      ...
      if (!sav[i]->sav_label_sync)
        continue;
      good_writes = kmem_zalloc(sizeof(uint64_t), KM_SLEEP);
      zio_t *vio = zio_null(zio, spa, NULL, vdev_label_sync_ignore_done,
                            good_writes, flags);
      vdev_label_sync(vio, good_writes, sav[i]->sav_vdevs[v], l, txg, flags);
      zio_nowait(vio);
    }
  }
 ...
 */
int vdev_config_sync(vdev_t **svd, int svdcount, uint64_t txg) {
  spa_t *spa = svd[0]->vdev_spa;
...
retry:
  ...
  if (ub->ub_txg < txg) {
    boolean_t changed =
        uberblock_update(ub, spa->spa_root_vdev, txg, spa->spa_mmp.mmp_delay);
  ...
  if (txg > spa_freeze_txg(spa)) {
    ...
    zfs_dbgmsg(" pool is frozen in spa_freeze_txg(spa)=%llu\
			(now at txg=%llu)\n",
               (u_longlong_t)spa_freeze_txg(spa), (u_longlong_t)txg);
    return (0);
  }
  ...
  for (vdev_t *vd = txg_list_head(&spa->spa_vdev_txg_list, TXG_CLEAN(txg));
       vd != NULL;
