...
#define SPA_GBH_NBLKPTRS                                                       \
  ((SPA_GANGBLOCKSIZE - sizeof(zio_eck_t)) / sizeof(blkptr_t))
#define SPA_GBH_FILLER                                                         \
  ((SPA_GANGBLOCKSIZE - sizeof(zio_eck_t) -                                    \
...
#define BOOTFS_COMPRESS_VALID(compress)                                        \
  ((compress) == ZIO_COMPRESS_LZJB || (compress) == ZIO_COMPRESS_LZ4 ||        \
   (compress) == ZIO_COMPRESS_GZIP_1 || (compress) == ZIO_COMPRESS_GZIP_2 ||   \
   (compress) == ZIO_COMPRESS_GZIP_3 || (compress) == ZIO_COMPRESS_GZIP_4 ||   \
   (compress) == ZIO_COMPRESS_GZIP_5 || (compress) == ZIO_COMPRESS_GZIP_6 ||   \
   (compress) == ZIO_COMPRESS_GZIP_7 || (compress) == ZIO_COMPRESS_GZIP_8 ||   \
   (compress) == ZIO_COMPRESS_GZIP_9 || (compress) == ZIO_COMPRESS_ZLE ||      \
   (compress) == ZIO_COMPRESS_ZSTD || (compress) == ZIO_COMPRESS_ON ||         \
...
#define ZIO_COMPLEVEL_ZSTD(level) ZIO_COMPRESS_RAW(ZIO_COMPRESS_ZSTD, level)
...
#define ZIO_DDT_CHILD_FLAGS(zio)                                               \
  (((zio)->io_flags & ZIO_FLAG_DDT_INHERIT) | ZIO_FLAG_DDT_CHILD |             \
   ZIO_FLAG_CANFAIL)

#define ZIO_GANG_CHILD_FLAGS(zio)                                              \
  (((zio)->io_flags & ZIO_FLAG_GANG_INHERIT) | ZIO_FLAG_GANG_CHILD |           \
   ZIO_FLAG_CANFAIL)

#define ZIO_VDEV_CHILD_FLAGS(zio)                                              \
  (((zio)->io_flags & ZIO_FLAG_VDEV_INHERIT) | ZIO_FLAG_DONT_PROPAGATE |       \
   ZIO_FLAG_CANFAIL)
...
#define ZIO_CHILD_ALL_BITS                                                     \
  (ZIO_CHILD_VDEV_BIT | ZIO_CHILD_GANG_BIT | ZIO_CHILD_DDT_BIT |               \
...
#define ZB_IS_ZERO(zb)                                                         \
  ((zb)->zb_objset == 0 && (zb)->zb_object == 0 && (zb)->zb_level == 0 &&      \
   (zb)->zb_blkid == 0)
#define ZB_IS_ROOT(zb)                                                         \
  ((zb)->zb_object == ZB_ROOT_OBJECT && (zb)->zb_level == ZB_ROOT_LEVEL &&     \
...
typedef zio_t *zio_gang_issue_func_t(zio_t *zio, blkptr_t *bp,
                                     zio_gang_node_t *gn, struct abd *data,
                                     uint64_t offset);
...
enum blk_verify_flag { BLK_VERIFY_ONLY, BLK_VERIFY_LOG, BLK_VERIFY_HALT };
...
extern zio_t *zio_root(spa_t *spa, zio_done_func_t *done, void *priv,
                       zio_flag_t flags);

extern void zio_destroy(zio_t *zio);

extern zio_t *zio_read(zio_t *pio, spa_t *spa, const blkptr_t *bp,
                       struct abd *data, uint64_t lsize, zio_done_func_t *done,
                       void *priv, zio_priority_t priority, zio_flag_t flags,
                       const zbookmark_phys_t *zb);

extern zio_t *zio_write(zio_t *pio, spa_t *spa, uint64_t txg, blkptr_t *bp,
                        struct abd *data, uint64_t size, uint64_t psize,
...
extern zio_t *zio_claim(zio_t *pio, spa_t *spa, uint64_t txg,
                        const blkptr_t *bp, zio_done_func_t *done, void *priv,
                        zio_flag_t flags);
...
extern zio_t *zio_vdev_child_io(zio_t *zio, blkptr_t *bp, vdev_t *vd,
                                uint64_t offset, struct abd *data,
...
extern zio_t *zio_vdev_delegated_io(vdev_t *vd, uint64_t offset,
                                    ...
                                    zio_flag_t flags, zio_done_func_t *done,
                                    void *priv);
...
extern int zfs_ereport_post_checksum(spa_t *spa, vdev_t *vd,
                                     const zbookmark_phys_t *zb,
                                     struct zio *zio, uint64_t offset,
                                     uint64_t length, const abd_t *good_data,
                                     const abd_t *bad_data,
                                     struct zio_bad_cksum *info);
